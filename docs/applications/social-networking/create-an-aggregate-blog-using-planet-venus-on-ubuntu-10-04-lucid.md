...
Copy the default configuration file to the `/srv/www/bucknell.net` directory:
    ...
    cp /usr/share/planet-venus/example/default.ini /srv/www/bucknell.net/planet.conf
...
:   ~~~
    ...
    output_theme = /srv/www/bucknell.net/planet-theme
    cache_directory = /srv/www/bucknell.net/planet-cache
...
Issue the following command to copy the default theme directory to the `/srv/www/bucknell.net/theme`:

    cp -R /usr/share/planet-venus/theme/default /srv/www/bucknell.net/planet-theme
...
Once you have completed all modifications to `planet.conf`, run Planet for the first time by issuing the following command:
    ...
    planet /srv/www/bucknell.net/planet.conf
...
fcrontab
:   ~~~
    */10* * * * planet /srv/www/bucknell.net/planet.conf
    ~~~
