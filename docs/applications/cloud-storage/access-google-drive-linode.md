...
Before beginning, you should be familiar with our guides on [getting started](/content/getting-started) and [securing your server](/content/security/securing-your-server), particularly if your Google Drive contains sensitive personal information. This guide is intended to be run as a non-root user, with sudo privileges required for some steps.
...
1.  Visit the Google API developer's console on your desktop at https://console.developers.google.com/project
    ...
    ![The Google Developers console.](/content/assets/drive_console.png)

2.  Create a project. Click **Create Project**, then give the project a name and click **Create** again:

    ![The new project window. I called my project Google Drive Linode](/content/assets/API_console_new_project.png)

    Google will take a moment to create the project, and when it's complete you'll arrive at the dashboard:

    [![The project "dashboard"](/content/assets/API-dashboard-small.png)](/content/assets/API_dashboard.png)
...
3.  Enable the Google Drive API. Click **APIs & auth**, then **APIs** when the menu expands. You'll see a list like the one below. Click on **Drive API**:
    ...
    [![The API list.](/content/assets/google_API_screen-small.png)](/content/assets/google_API_screen.png)
    ...
    ![The Google Drive API description.](/content/assets/drive_enable_API.png)

4.  Click **Credentials** in the menu on the left side of the page. Then click **Add New Credential**.

    ![The credentials screen.](/content/assets/new_oauth2.jpg)

    Click **Configure consent screen**. Google assumes you are writing a piece of software so it wants some information about it.

    [![Creating a client ID.](/content/assets/new_configure_screen_small.jpg)](/content/assets/new_configure_screen.jpg)

    The product name field is required, but you can leave everything else blank. Then click **Save** at the bottom of the page.

    ![Configuring the Google Drive API project. I named mine "Google Drive Linode link"](/content/assets/new_product_name.jpg)
    ...
    ![Selecting where this API key will be used](/content/assets/new_other_application.jpg)
    ...
    ![The Client ID and Client secret strings generated by google.](/content/assets/new_credentials.jpg)
...
2.  Google will ask for permission to allow this new application (OCamlfuse) to access your Google Drive. Click **Accept** to receive the verification code:
    ...
    ![The permission request screen from google.](/content/assets/google_authorization.png)
