...
1.  Open [PuTTY](/content/networking/using-putty) and navigate under the `SSH` menu to `Tunnels`. Add a new forwarded port as shown below, replacing example.com with your Linode's IP address or hostname:
...
4.  After connecting you will be greeted with a terminal emulator window.
    ...
    [![VNC connection to a terminal emulator.](/content/assets/1646-vnc-4_small.png)](/content/assets/1639-vnc-4.png)
...
2.  Click the button to `Create a new remote desktop profile`. Name your profile, specify the VNC protocol, and enter localhost :1 in the server field. Be sure to include the`:1` in the `Server` section. In the password section fill in the password you specified in Step 4 of [the previous section](#installing-a-desktop-and-vnc-on-your-linode):
    ...
    [![.](/content/assets/1641-vnc-ubuntu-2.png)](/content/assets/1641-vnc-ubuntu-2.png)

3.  Press **Connect**.

    [![An Ubuntu desktop computer connected to an Ubuntu desktop session on a Linode.](/content/assets/1644-vnc-ubuntu-3-1_small.png)](/content/assets/1645-vnc-ubuntu-3-1.png)
