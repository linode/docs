...
description: 'Outline of the memcached amplification attack and how it affects you.'
...
Memcached should not be exposed to the internet. If your Linode is detected to be running a vulnerable instance of memcached, you will be alerted by our Support team. You can of course preemptively take steps to secure your memcachd instance.

- If you require memcached to communicate over a private network, you should:

    1.  Bind memcache to a specific IP address and port.

    2.  Firewall your Linode appropriately. If you need help with configuring a firewall in your distribution, see [our firewall guides](/docs/security/firewalls/) for more information.

- If you do not require memcached to communicate over any network connection, you should disable UDP for memcached and ensure it is only listening on *localhost*:
...
1.  Memcached is configured by default to listen for incoming connections from any IP address on RHEL-based distributions, on both TCP and UDP. Change this to only listen on localhost, and as an added precaution, disable UDP for memcached:
    ...
    Edit the `OPTIONS` line in the memcache config file:
...
1.  Memcached is configured to listen on localhost by default in Debian and Ubuntu. This configuration is not vulnerable to the amplification exploit, but you should still verify your setup:
    ...
    Memcache should again show that it's listening on *localhost*, or address `127.0.0.1`, as shown below:
...
2.  Disable memcache's use of UDP as an additional precaution:
