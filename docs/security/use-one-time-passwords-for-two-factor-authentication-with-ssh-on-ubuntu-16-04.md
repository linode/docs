...
keywords: 'two factor authentication,ssh,google authenticator,ubuntu,debian,ssh two factor authentication,two factor authentication ssh key'
...
Now that the package has been installed, we'll use it to generate keys. These keys are then used by software on client devices to generate time-based one-time passwords. To understand the difference between these passwords and the ones you already use, let's break down the concept of a TOTP:
...
1.  Open `/etc/pam.d/sshd` with sudo privileges and add these lines to the end of the file:
