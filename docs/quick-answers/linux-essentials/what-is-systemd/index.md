...
systemd is a Linux initialization system and service manager that includes features like, on demand starting of daemons, mount and automount point maintenance, snapshot support, processes tracking using Linux control groups, and provides a logging daemon and other tools and utilities to help with common system administration tasks.

[Lenart Poetttering](http://0pointer.de/blog/projects/systemd.html) and [Kay Sievers](https://en.wikipedia.org/wiki/Kay_Sievers) wrote systemd with the goal of creating a modern and dynamic system that was inspired by [launchd](https://en.wikipedia.org/wiki/Launchd) --a macOS service management framework-- and [Upstart](http://upstart.ubuntu.com/) --an initialization system that provides service management. Notably, systemd provides aggressive parallelization capabilities, leading to a quicker boot time, and dependency-based service control logic, which allows for services to start in parallel. These two aspects were present in Upstart, but improved upon by systemd.

systemd is the default init system for all of the major Linux distributions. It is backwards compatible with SysV init scripts. SysVinit is an initialization system that predates systemd and uses a simplified approach to service startup. systemd not only manages system initialization, but also provides alternatives for other well known utilities, like cron and syslog. Because systemd does several things within the Linux user space, many have criticized it for violating [the Unix philosophy](https://en.wikipedia.org/wiki/Unix_philosophy), which emphasizes simplicity and modularity.
...
To better understand what is meant by an initialization system, this section will provide a high-level overview of the Linux boot process.

Linux requires an initialization system during its boot and startup process. At the end of the [boot process](https://en.wikipedia.org/wiki/Linux_startup_process), the Linux kernel loads systemd and passes control over to it and the startup process begins. During this step, the kernel initializes the first [user space](https://en.wikipedia.org/wiki/User_space) process, the systemd init process with process ID 1, and then goes idle unless it is called. systemd prepares the [user space](https://en.wikipedia.org/wiki/User_space) and brings the Linux host into an operational state by starting all other processes that run on the Linux system.

Below is a simplified overview of the entire Linux boot and startup process:

1. After power up of the system, the BIOS does minimal hardware initialization and hands over control to the boot loader.
...
1. systemd takes over and continues to mount the host's file systems and services.

## systemd Units

systemd introduces the concept of *systemd units*. There are several systemd unit types, like a *service unit*, *mount unit*, *socket unit* and *slice unit*. Units are defined in unit configuration files, which include information about the unit type and its behavior.
...
On an Ubuntu 18.04 distribution, unit files are stored in the following directories:
...
To view information about a specific mount unit, use systemctl's status command with the unit's name:
...
Systemctl's output provides the location of the mount unit configuration file, along with information on the state of the mount, location of the mount, links to documentation, running tasks, and its corresponding CGroup. Many of these details are defined in the mount unit's configuration file.

[FUSE](http://libfuse.github.io/doxygen/) is a filesystem framework that provides an interface for userspace programs to export a virtual filesystem to the Linux kernel. It can also be used to provide data access with a file system directory structure and file operations to any object.

Inspect the contents of the FUSE control filesystem's mount unit configuration file with systemctl:
...
- There are no service dependencies for this mount unit, indicated by `DefaultDependencies=no`, however, `ConditionPathExists` denotes that the directory `/sys/fs/fuse/connections` must exist before the mount unit is started. This condition is necessary, since under the FUSE control filesystem each connection will have it's own directory in this location.
...
{{< note >}}
Your script, service unit file, timer unit file should all have `644` read and write permissions.
{{</ note >}}
