...
If you wish to deploy Mailman on a system that has an existing mail set up, such as the [Postfix with Courier and MySQL](/content/email/postfix/courier-mysql-ubuntu-9-10-karmic) or [Postfix with Dovecot and MySQL](/content/email/postfix/dovecot-mysql-ubuntu-9-10-karmic). configurations described in other documents, consider the following recommendations:
