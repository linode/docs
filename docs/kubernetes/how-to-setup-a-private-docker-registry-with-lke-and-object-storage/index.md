...
h1_title: "Set Up a Private Docker Registry with Linode Kubernetes Engine and Object Storage"
contributor:
  name: Leslie Salazar
  link: https://github.com/leslitagordita/
---
Hosting a private Docker registry alongside your Kubernetes cluster allows you and your team to securely manage your Docker images while also providing quick deployment of your apps. This guide will walk you through the steps needed to deploy a private docker registry on a Linode Kubernetes Engine (LKE) cluster. These steps will allow you to locally push and pull Docker images to your registry and similarly, your cluster's pods will be able to pull Docker images from the registry to complete their deployments.
...
1. [Deploy a LKE Cluster]. This example was written using a node pool with two [2 GB nodes](https://www.linode.com/pricing/). Depending on the workloads you will be deploying on your cluster, you may consider using nodes with higher resources.

1. Install [Helm 3](/docs/kubernetes/how-to-install-apps-on-kubernetes-with-helm-3/#install-helm), [kubectl](/docs/kubernetes/deploy-and-manage-a-cluster-with-linode-kubernetes-engine-a-tutorial/#install-kubectl), and [Docker](https://docs.docker.com/get-docker/) to your local environment.
...
1. Purchase a domain name from a reliable domain registrar. Using Linode's DNS Manager, [create a new Domain](/docs/platform/manager/dns-manager/#add-a-domain) and [add an "A" DNS record](/docs/platform/manager/dns-manager/#add-dns-records) for a subdomain named `registry`. Your subdomain will host your Docker registry. This guide will use `registry.example.com` as the example domain.
...
Now that your NGINX Ingress Controller has been deployed you are ready to enable HTTPS on your Docker registry.
...
You are now ready to deploy your Docker Registry to your cluster using a Helm chart.
...
1. Create a new file named `docker-configs.yaml` using the example configurations. Ensure you replace the following values:
...
To enabled basic access restriction for your Docker registry, you will use the `htpasswd` utility. This utility allows you to use a file to store usernames and passwords for basic HTTP authentication.
...
1. Create a file to store your username and password.
...
1. Create a secret to store your registry's authentication information. Replace the option values with your own registry's details. The `--docker-username` and `--docker-password` should be the username and password that you used when generating a credentials using the `htpasswd` utility.
...
## Push and Pull Images to your Docker Registry

You are now ready to push and pull images to your Docker registry. In this section you will pull an existing image from Docker Hub and then push it to your registry. In the next section you will use your registry's image to deploy an example static site.
...
1. Tag the image. Ensure that you replace `registry.example.com` with your own registry's domain.
...
In this section, you will create test deployment using the image that you pushed to your registry in the previous section. This will ensure that your cluster can authenticate to your Docker registry and pull images from it.

1. Using Linode's DNS manager, [create a new subdomain A record](/docs/platform/manager/dns-manager/#add-dns-records) to host your static site. The example will use `static.example.com`. When creating your record, as the IP address, assign it your cluster's NodeBalancer external IP address. You can find the external IP address with the following command:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      - In the Deployment section of the manifest, the [`imagePullSecrets` field](https://kubernetes.io/docs/concepts/configuration/secret/#using-imagepullsecrets) references the secret you created that contains the authentication credentials that your cluster's kubelet can use to pull your private registry's image. While, the `image` field provides the image to pull from your Docker registry.
