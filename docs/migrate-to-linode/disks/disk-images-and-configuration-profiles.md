...
6.  Click **Duplicate Image**. The Linode's dashboard appears. Watch the *Host Job Queue* for confirmation that the disk has been duplicated.
...
8.  In the *Block Device Assignment* section, select a bootable disk, a swap image (optional), and any other disks that you would like to mount at start up.
