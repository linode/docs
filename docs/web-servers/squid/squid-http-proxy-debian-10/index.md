...
description: This guide show how to use Squid to create an HTTP proxy server on your Linode running Debian 10.
og_description: This guide show how to use Squid to create an HTTP proxy server on your Linode running Debian 10.
...
1. Edit the Squid configuration file and add the following lines at the beginning of the file:
    ...
    {{< note >}}Ensure that you update `/usr/lib/squid/basic_ncsa_auth` below with the location of the `nsca_auth` file.{{< /note >}}
    {{< file "/etc/squid/squid.conf" >}}
