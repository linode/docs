...
aliases: ['web-servers/nginx/python-uwsgi/ubuntu-10-10-maverick/','websites/nginx/wsgi-using-uwsgi-and-nginx-on-ubuntu-10-10-maverick/']
modified: 2012-10-08
modified_by:
  name: Linode
...
Issue the following commands to download and compile nginx with support for the `uwsgi` protocol. If you previously installed nginx from Ubuntu packages, remove them at this juncture. The following command sequence mirrors the procedure defined in the [installation guide for nginx](/content/web-servers/nginx/installation/ubuntu-10-10-maverick) for compiling nginx from source:
    ...
    wget -O init-deb.sh http://www.linode.com/content/assets/688-init-deb.sh
...
Issue the following command to download an init script to manage the uWSGI process, located at `/etc/init.d/uwsgi`:
    ...
    cd /opt/
    wget -O init-deb.sh http://www.linode.com/content/assets/689-uwsgi-init-deb.sh
...
- [Installing Nginx on Ubuntu 10.10 (Maverick)](/content/web-servers/nginx/installation/ubuntu-10-10-maverick)
- [Deploy a LEMP Server on Ubuntu 10.10 (Maverick)](/content/lemp-guides/ubuntu-10-10-maverick/)
