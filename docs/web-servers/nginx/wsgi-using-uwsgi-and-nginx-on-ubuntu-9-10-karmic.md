...
Issue the following commands to download and compile nginx with support for the `uwsgi` protocol. If you previously installed nginx from Ubuntu packages, remove them at this juncture. The following command sequence mirrors the procedure defined in the [installation guide for nginx](/content/web-servers/nginx/installation/ubuntu-9-10-karmic) for compiling nginx from source:
    ...
    wget -O init-deb.sh http://www.linode.com/content/assets/652-init-deb.sh
...
Issue the following command to download an init script to manage the uWSGI process, located at `/etc/init.d/uwsgi`:
    ...
    cd /opt/
    wget -O init-deb.sh http://www.linode.com/content/assets/651-uwsgi-init-deb.sh
...
- [Deploy a LEMP Server on Ubuntu 9.10 (Karmic)](/content/lemp-guides/ubuntu-9-10-karmic/)
