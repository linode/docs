...
Before you begin to use the nginx server, you must create a means of controlling it. You can create an "init script" using [this example](/content/assets/663-init-rpm.sh) to control nginx. Issue the following commands to download the file, change the execution mode, and set the system to initialize nginx on boot:

    wget -O init-rpm.sh http://www.linode.com/content/assets/663-init-rpm.sh
