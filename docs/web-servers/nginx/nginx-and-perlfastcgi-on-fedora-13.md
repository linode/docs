...
aliases: ['web-servers/nginx/perl-fastcgi/fedora-13/','websites/nginx/nginx-and-perlfastcgi-on-fedora-13/']
modified: 2011-05-17
modified_by:
  name: Linode
...
Issue the following command sequence to download the FastCGI wrapper script (credit: [Denis S. Filimonov](http://www.ruby-forum.com/topic/145858)) and an init script to control the FastCGI process, set the permissions, launch the wrapper for the first time, and ensure that FastCGI launches at startup:
    ...
    cd /opt/
    wget -O fastcgi-wrapper http://www.linode.com/content/assets/640-fastcgi-wrapper.sh
    wget -O init-rpm.sh http://www.linode.com/content/assets/639-init-rpm.sh
