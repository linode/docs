...
aliases: ['web-servers/nginx/python-uwsgi/debian-6-squeeze/','websites/nginx/wsgi-using-uwsgi-and-nginx-on-debian-6-squeeze/']
modified: 2011-05-09
modified_by:
  name: Linode
...
Issue the following commands to download and compile nginx with support for the `uwsgi` protocol. If you previously installed nginx from Debian packages, remove them at this juncture. The following command sequence mirrors the procedure defined in the [installation guide for nginx](/content/web-servers/nginx/installation/debian-6-squeeze) for compiling nginx from source:
    ...
    wget -O init-deb.sh http://www.linode.com/content/assets/682-init-deb.sh
...
Issue the following command to download an init script to manage the uWSGI process, located at `/etc/init.d/uwsgi`:
    ...
    cd /opt/
    wget -O init-deb.sh http://www.linode.com/content/assets/683-uwsgi-init-deb.sh
