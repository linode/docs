...
| `TEST` | testing | Here's a table within the table: <table><thead><tr><th>Test</th><th>Table</th><th>In</th><th>Table</th></tr></thead><tbody><tr><td>Testing</td><td>Tables</td><td>within</td><td>tables</td></tr><tr><td>All</td><td>The</td><td>Things</td><td>This row a lot of content to see what happens `curl https://api.linode.com/v4/linode/types`. - if it gets wide, it just makes the outside table wider and that now scrolls. I also want to test thing like bulleted lists, this worked above...</br></br><ul><li>here's an ul element with nested li elements</li><li>list item #2</li></ul></td><tr><td>UL</td><td>VS</td><td>`&bull;`</td><td>I like ul better than using the `&bull;` above because it automatically handles indenting and line spacing. I did change the font-size of the li element within the definition-table. When used in a regular table the font size will be the same as body text, not table text unless you want me to make a sweeping all-table change.</td></tr></tbody></table>Here's a test of just the list ul and li to see if it works in the standard definition-table: <ul><li>item number 1 goes here</li><li>item number 2 goes here</li></ul>It looks like almost any html code will work here. Spans: <span style="color:green;">this is a styled span with specific font color specified</span></br></br>It doesn't appear that you can use other shortcodes inside of this one...yet. I'll look into it, we can do for disclosure notes, if this is wanted, let me know. |
{{< /definition-table >}}
