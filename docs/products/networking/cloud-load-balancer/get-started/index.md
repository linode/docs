...
| **Endpoints** | The port, IP or compute instance must differ. Capacity and health status are taken into consideration in the endpoint selection process. Endpoints for a service target must be unique.|
...
|Protocol| HTTP or TCP.  You can select TCP health checks even when the load balancers entry point protocol is HTTP or HTTPS.
