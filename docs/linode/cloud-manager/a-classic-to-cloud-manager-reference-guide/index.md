...
contributor:
  name: Linode
---
Linode's Classic Manager is being [retired on January 31, 2020](https://www.linode.com/2019/11/18/notice-legacy-linode-manager-users/). At that time, all users will be migrated to the new [Cloud Manager](https://cloud.linode.com/) when logging in to manage your infrastructure on Linode.
...
- [Summary information](/docs/uptime/monitoring-and-maintaining-your-server/#linode-cloud-manager) about your Linode, like CPU usage, IPv4 and IPv6 traffic, and Disk IO
...
### Find Volumes
In Classic Manager, Volumes were found within a Linode's Dashboard. In Cloud Manager, Volumes are their own top-level menu item in the sidebar.
...
To learn how to begin using Object Storage, view our [How to Use Linode Object Storage](/docs/platform/object-storage/how-to-use-object-storage/). To access all our Object Storage guides, see the [Object Storage section](/docs/platform/object-storage/) of our documentation site.
...
## Domains (DNS Manager)
In Classic Manager, the DNS Manager is where you can control your domains. In Cloud Manager, the DNS Manager is located in the **Domains** link in the sidebar.
...
While Cloud Manager does not include the "Check Zone" and "Zone File" features, it ensures that your Domain's zone file is created correctly:

- When creating a Zone File for a Domain, the Linode API v4 checks for any errors that may exist. If an error is found, the Cloud Manager will respond with the corresponding error. This means that the Cloud Manager will never allow you to create an invalid zone file.

- Once your Domain and corresponding Zone File are created, you can use the `dig` command to further verify that each domain record represents the information you expect, for example:
...
To get started using LKE, see our [A Tutorial for Deploying and Managing a Cluster with Linode Kubernetes Engine](/docs/kubernetes/deploy-and-manage-a-cluster-with-linode-kubernetes-engine-a-tutorial/).

## StackScripts

[StackScripts](https://linode.com/stackscripts/) provide Linode users with the ability to automate the deployment of custom systems on top of our default Linux distribution images. Linodes deployed with a StackScript run the script as part of the first boot process.
...
In Classic Manager, StackScripts were found in the Linodes tab. In Cloud Manager, StackScripts are their own top-level menu item in the sidebar.
...
### Find Images
In Classic Manager, Manage Images was found in the Linodes tab. In Cloud Manager, Images are their own top-level menu item in the sidebar.
