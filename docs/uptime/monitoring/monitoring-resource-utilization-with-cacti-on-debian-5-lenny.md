...
aliases: ['server-monitoring/cacti/debian-5-lenny/']
modified: 2012-10-08
modified_by:
  name: Linode
...
You will need to create a password for the `root` user of your MySQL database during the installation. After the installation completes, be sure to run `mysql_secure_installation` to disable some of MySQL's less secure components. Also consider reading our [MySQL installation guide](/content/databases/mysql/debian-5-lenny) for configuration recommendations.

The above command will additionally install the Apache web server. Consider our documentation of [installing the Apache HTTP Server](/content/web-servers/apache/installation/debian-5-lenny) for more information regarding this server. Additionally Cacti can function with alternate web server configurations, including [Apache with PHP running as a CGI process](/content/web-servers/apache/php-cgi/debian-5-lenny) and with [nginx](/content/web-servers/nginx/) running PHP as a FastCGI process.
...
- [Configure a Basic Firewall in Debian 5 (Lenny)](/content/security/firewalls/arno-iptables-debian-5-lenny)
