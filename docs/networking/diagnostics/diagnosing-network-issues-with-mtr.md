...
>
> It can also be useful to run a report at a faster rate to reveal congested network problems. The `i` option which instructs MTR to send one packet ever `n` seconds. The default is 1, so setting it lower is generally helpful.
