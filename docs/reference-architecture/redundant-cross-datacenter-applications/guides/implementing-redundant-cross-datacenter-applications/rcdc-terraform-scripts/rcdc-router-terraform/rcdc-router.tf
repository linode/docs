...
resource "linode_instance" "rcdc_router" {
  count = var.node_count
  image = "linode/ubuntu20.04"
  label = "rcdc-router-${var.regions[count.index]}-${count.index + 1}"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # In this case, and output of 871e0899a6 was fit to the fd00::/8 range
      # reserved for private IPv6 addresses.
      "cat > /etc/wireguard/wg0.conf <<EOF",
      "[Interface]",
      "PrivateKey = $(cat /etc/wireguard/private.key)",
      "Address = 10.2.0.${count.index + 1}/24",
      "ListenPort = 51820",
