...
This guide describes the recommended strategy for migrating your services from another host to Linode. The specific steps you'll need to carry out will vary by why kind of software you use, but the high-level outline is generally the same regardless of the nature of your service. The [Migrate to Linode](/docs/platform/migrate-to-linode) section offers further guides which describe migrating particular services in more detail.
...
-   To determine which data center to locate in, run speed tests to the different regions that Linode offers from the [speedtest page](https://www.linode.com/speedtest). This page allows you to download a 100MB file from each location, and you can compare the speed of each download to determine the bandwidth between your location and the data center.
...
-   To determine which plan to choose, review the [Linode Pricing page](https://www.linode.com/pricing#all). At a minimum, you should choose a plan which offers enough storage capacity for the data you store on your current hosting provider. The CPU and RAM allocations are also important, as a service with a higher workload/higher traffic will require more of each. If you're not sure what your workload will require, you can always start with a smaller Linode and then [resize your plan](docs/platform/disk-images/resizing-a-linode/) upwards or downwards as needed.
...
For further detail on deploying your new Linux image, follow the [Getting Started with Linode](/docs/getting-started/) guide. It is also recommended that you follow the [How to Secure Your Server](/docs/security/securing-your-server/) guide once you have deployed your new image.
...
- Which software configuration settings should be preserved (e.g. web server virtual host information, database connection settings, which files that contain those settings, etc).
...
-   [Create Physical Backups of your MariaDB or MySQL Databases](https://www.linode.com/docs/databases/mysql/create-physical-backups-of-your-mariadb-or-mysql-databases/)
...
If your current host is a shared environment and you do not have full administrative/command-line access to it, then your host may offer an alternative method for exporting your data. If this is the case, then you should use those tools to download the data to your computer or some other location.

### Transfer Your Data to Your Linode

Transfer your data to your Linode using a network transfer tool like `rsync`: [Introduction to rsync](https://www.linode.com/docs/tools-reference/tools/introduction-to-rsync/). For example, the following command will upload files from `/path/to/source_folder` on the current host to `/path/to/destination_folder` on the new Linode. Run this command from your current host:
...
If your current host is a shared environment and you previously downloaded your data to your computer, then you should upload the data from your computer to your Linode. You can use an SFTP tool like [FileZilla](http://localhost:60879/docs/tools-reference/file-transfer/filezilla/) to do this, which has clients available for Windows, Mac, and Linux.
...
To direct your service's visitors to Linode, you will need to associate your domain with [your new Linode's IP](/docs/networking/remote-access/#network-access). There are two options for moving your DNS records:
