...
![The web application when the guide starts.](/content/assets/806-starting-point.png)
...
![Diagram after putting NodeBalancer in charge of the web application.](/content/assets/807-first-step.png)

Now, backend Linodes can be added or removed to the work load seamlessly, without end users noticing any downtime.

![Diagram after adding a second web server node.](/content/assets/808-scaling.png)
...
Sticking with the simple web application example above, the backend Linode currently powering it must have a private IP address. If it does not, add one now and configure the Linode for [static networking](/content/networking/configuring-static-ip-interfaces/) -- this will also be a good opportunity to do the same for the database server, since private network traffic is unbilled. Reboot if necessary, and configure your web application to respond on the private IP address, if it is not already.

1.  Visit the NodeBalancers tab in the Linode Manager.

	[![The NodeBalancer tab.](/content/assets/796-1.png)](/content/assets/770-nodebalancer-tab.png)
...
2.  For the example web application, only one NodeBalancer is needed. Add one in the same datacenter that your backend Linodes are located in. Once purchased, you will be able to see the public IP address that has been assigned to your NodeBalancer.
	...
	[![The NodeBalancer has been added.](/content/assets/797-2.png)](/content/assets/772-nodebalancer-added.png)
...
3.  Now choose **Create Configuration**. A NodeBalancer is configured using ports, and let's say our example web application uses only one: port 80 for regular HTTP traffic.
	...
	[![Adding a port configuration to a NodeBalancer.](/content/assets/798-3.png)](/content/assets/774-add-port.png)
	...
	**HTTPS**
	...
	For all other implementations, the default **Recommended** cipher suite option should be used. You can see the cipher suites available with each option in our [NodeBalancer Reference Guide](/content/platform/nodebalancer/nodebalancer-reference-guide#tls-cipher-suites).

	[![SSL Cipher Suite](/content/assets/ssl-cipher-suite-resized.png)](/content/assets/ssl-cipher-suite.png)
...
1.  Now we must add the single backend node to the NodeBalancer's configuration. Point this at the private IP address of your web server Linode.
	...
	[![Adding a backend node to a NodeBalancer.](/content/assets/799-4.png)](/content/assets/776-backend.png)
	...
	[![The backend node has been added, and is now status UP.](/content/assets/800-5.png)](/content/assets/778-backend-up.png)
...
2.  Now that the backend is up, go directly to your NodeBalancer's IP address in a browser. You should see your web application as the NodeBalancer proxies the traffic through.
	...
	[![Viewing the NodeBalancer-driven web site in a browser.](/content/assets/801-6.png)](/content/assets/780-success.png)
...
2.  Edit or create an A record for `www.example.org`, pointing to your NodeBalancer's IP address.
	...
	[![Adding an A Record.](/content/assets/nodebalancer-a-record_small.png)](/content/assets/nodebalancer-a-record.png)

	Also add an AAAA record for the IPv6 address.

	[![Adding an AAAA record for the NodeBalancer.](/content/assets/804-9.png)](/content/assets/786-dns-aaaa.png)
...
On another Linode, make an exact copy of your current web server. The Linode backups service can be instrumental for doing so, as a snapshot can be restored to any other Linode. Once you have another backend ready, simply repeat the steps for [Adding a Backend](/content/platform/nodebalancer/getting-started-with-nodebalancers#adding-a-backend) to add it to the NodeBalancer configuration.

[![Adding another backend to the NodeBalancer's configuration.](/content/assets/805-10.png)](/content/assets/788-another-backend.png)
