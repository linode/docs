...
httpd.conf
    ...
    > \<IfModule mod\_status.c\> ExtendedStatus On \<Location /server-status\> SetHandler server-status Order deny,allow Deny from all Allow from 127.0.0.1 \</Location\> \</IfModule\>
