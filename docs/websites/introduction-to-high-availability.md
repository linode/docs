...
The concepts discussed here are specifically geared toward the configuration described in our guide on how to [host a website with high availability](/content/websites/host-a-website-with-high-availability), but will apply to highly available systems in general. The  diagram below shows the configuration we use in our guide.

[![High availability server configuration](/content/assets/high-availability-diagram.png)](/content/assets/high-availability-diagram.png)
...
Our solution to load balancing is the [NodeBalancer](/content/platform/nodebalancer/getting-started-with-nodebalancers), a highly available component that will evenly distribute incoming traffic to one of the three application servers, ensuring that no single server experiences a much heavier load than the others.
...
Now that you understand high availability, you can begin to apply its concepts when designing your own systems and setting up your own hosting configurations. If you're ready to get started right away, check our guide to [hosting a website with high availability](/content/websites/host-a-website-with-high-availability) for instructions on how to set up the system described above.
