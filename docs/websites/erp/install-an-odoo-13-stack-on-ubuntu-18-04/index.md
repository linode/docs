...
keywords: ["Odoo", "Odoo ERP", "CMS", "Ubuntu", "CRM", "OpenERP", "Odoo 13", "Ubuntu 18.04"]
...
![Install an Odoo 13 Stack on Ubuntu 18.04 using Linode](install-an-odoo-13-stack-on-ubuntu-18-04-using-linode.png "Odoo 13 on Ubuntu 18.04 Title Graphic")
...
For simple installations, Odoo and its dependencies can be installed on a single Linode (see our [Install Odoo 10 on Ubuntu](/docs/websites/cms/install-odoo-10-on-ubuntu-16-04/) guide for details). However, this single-server setup is not suited for production deployments. This guide covers how to configure a production Odoo 13 cluster in which the Odoo server and PostgreSQL database are hosted on separate Linodes hence giving you more flexibility and scalability. Furthermore, the cluster setup allows you using PostgreSQL database replication for added performance and reliability if you need to (see our [Install an Odoo 11 Stack on Ubuntu 16.04 using Linode](https://www.linode.com/docs/websites/erp/install-an-odoo-11-stack-on-ubuntu-16-04) guide for details).

## System Requirements

The setup in this guide requires the following *minimal* Linode specifications:

* A Standard **2GB** Linode to install the PostgreSQL 10 database
* A Nanode **1GB** Linode to install Odoo 13 web application
...
Ports `22`, `80`, and `5432` are the defaults for SSH, HTTP and PostgreSQL communications respectively. Port `6010` is used for Odoo communications and port `8069` is used by Odoo's webserver. To open a particular port you can use:
...
Configure Odoo's `postgresql` database backend Linode. Ubuntu 18.04 official repository includes PostgreSQL version 10 which offers significant performance improvements as well as database replication compatibility.
...
3. In a browser, navigate to `odoo.yourdomain.com:8069` or `http://<your_Linode_IP_address>:8069`. If your proxy and your DNS configuration are working properly a login screen will appear.

4. If everything works as expected, you will be presented with Odoo's database creation screen:

    ![Odoo 13 Database Screen](odoo-13-first-screen.png "Odoo 13 Initial Screen")

5.  Fill in all fields, check the **Demo data** box to populate your database with sample data  and then click on **Create database** button.
...
1. You can install PostgreSQL 10 on the **odoo** server using the procedure described on this guide. This will install `pg_dump` and other utilities, allowing you to use the Odoo GUI as before. Since Odoo configuration is explicit about database connection you will not have to worry about anything else. This method will restore the database to the **postgresql** server rather than **odoo**.
...
{{< note >}}
From Odoo version 12 is suggested that you update modules using Odoo's web interface whatever possible.
{{< /note >}}
