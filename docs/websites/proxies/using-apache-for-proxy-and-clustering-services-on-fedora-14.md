...
This guide provides a number of configuration examples and suggestions for using Apache as a frontend server for other HTTP servers and clusters of servers. If you have not already installed Apache, consider our documentation on [installing Apache](/content/web-servers/apache/installation/fedora-14) before continuing with this guide. Additionally, consider our [getting started](/content/getting-started/) and [beginner's guide](/content/beginners-guide/) documents if you are new to Linode, and our [administration basics](/content/using-linux/administration-basics) guide if you are new to Linux server administration.
...
In our guide to using [multiple web servers with ProxyPass](/content/web-servers/apache/proxy-configuration/multiple-webservers-proxypass-fedora-14) we outline a method for configuring multiple websites using Apache's `mod_proxy` module. Please follow that guide, particularly the section regarding [configuring mod\_proxy](/content/web-servers/apache/multiple-web-servers-with-proxypass-on-fedora-14/#enabling-the-proxy-module) to ensure that `mod_proxy` is active.
...
Before continuing, ensure that you've completed the ProxyPass guide, particularly the section regarding [configuring the proxy module](/content/web-servers/apache/proxy-configuration/multiple-webservers-proxypass-fedora-14). Do not omit to create and configure the `/etc/httpd/conf.d/proxy.conf` file.

Once `mod_proxy` is enabled and [configured properly](/content/web-servers/apache/proxy-configuration/multiple-webservers-proxypass-fedora-14), you can insert the following directives into your virtual hosting configuration.
...
Ensure that you have a `/etc/httpd/conf.d/proxy.conf` file as described in [this documentation](/content/web-servers/apache/multiple-web-servers-with-proxypass-on-fedora-14/#enabling-the-proxy-module). Do not omit to reload Apache again once you have fully configured your virtual host and cluster. Consider the following Apache configuration directives:
