...
modified_by:
  name: Linode
title: "How to Install a LAMP Stack on Ubuntu 22.04 | Linode"
...
The core of the [Ubuntu](https://ubuntu.com/server) open source architecture is the [LAMP Stack](https://en.wikipedia.org/wiki/LAMP_(software_bundle)). This stack includes an operating system, web server, programming language, and database. These applications are collectively able to implement web applications and other computing solutions. This guide provides some background about the LAMP stack and explains how to install and configure it on Ubuntu. It also explains how to quickly test interactions between the applications.

## What is a LAMP Stack?

The LAMP Stack is an acronym for Linux, [Apache](https://httpd.apache.org/docs/2.4/), [MySQL](https://dev.mysql.com/), and [PHP](https://www.php.net/). It includes an operating system, a web server, a database, and a versatile programming language. This software stack is the foundation of the open source Linux system. All components are available through the default Ubuntu software repositories.
...
This section explains how to install a LAMP Stack on Ubuntu 22.04. These instructions are also generally valid for Ubuntu 20.04. The stack can be installed either as individual components, or collectively using Tasksel.
...
### Installing the LAMP Stack using Tasksel

Tasksel is a utility found on Ubuntu and Debian systems for simplifying LAMP stack installation. Tasksel is able to simultaneously install multiple packages. This can speed up the installation. It also selects the release of each component to maximize compatibility. Tasksel should only be used if the LAMP stack components have not been installed yet. Conflicts have been known to occur when some parts of the stack are already present.

To install the LAMP Stack on Ubuntu using Tasksel, follow these instructions.

1. Ensure the Ubuntu packages are up to date using `apt`. Reboot the system if necessary.

        sudo apt update && sudo apt upgrade

2.  Install the `tasksel` package.

        sudo apt install tasksel

3.  Use the Tasksel utility to install the LAMP stack as a single package.

        sudo tasksel install lamp-server

### Installing the LAMP Stack Components Separately

The LAMP stack can also be installed without Tasksel. In this case, the individual parts of the stack can be upgraded separately later on. To install the LAMP stack using this approach, follow these steps. In all cases, enter `y` to proceed with the installation when Ubuntu asks for confirmation.
...
5.  Enable the `mpm_prefork` module and disable the `mpm_event` module using the `a2dismod` and `a2enmod` commands. Depending on the installation, these settings might already be configured.
...
Ensure these variables are not commented out. If necessary, remove the `;` character at the start of the line.
    ...
    {{< file "/etc/php/8.1/apache2/php.ini" aconf >}}
...
## A Summary of How to Install a LAMP Stack on Ubuntu

The Ubuntu LAMP Stack consists of the Linux operating system, Apache web server, MYSQL RDBMS, and PHP programming language. These free, open source components power many modern web applications. The individual components are designed to work together and are easy to install and use.

The LAMP stack can be installed on Ubuntu using `apt` or the Tasksel utility. After configuring the Apache web server, it is good practice to create a virtual host for the domain. To integrate the MySQL web server, create a new account to represent the web user. Additional PHP packages must be installed so Apache, PHP, and the database can communicate. The new installation can be tested using a short PHP test script that connects to the database. For more information about each LAMP Stack component, see the **More Information** section of this guide.