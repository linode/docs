...
description: "Apache is an open-source web server, the most widely-used on Linux systems, with a modular design makes it highly adapabtable. This guide shows you how to install Apache on Ubuntu 20.04 and how to get started using it."
og_description: "Apache is an open-source web server, the most widely-used on Linux systems, with a modular design makes it highly adapabtable. This guide shows you how to install Apache on Ubuntu 20.04 and how to get started using it."
...
h1_title: "How to Install and Use the Apache on Ubuntu 20.04"
contributor:
  name: Nathaniel Stickman
...
1. Install Apache from the package manager:
...
1. To have Apache begin running at system startup, enable the Apache service:
...
1. View the current status of the Apache service with:

        sudo systemctl status apache2

1. To stop the Apache service, use:

        sudo systemctl stop apache2

    You can then start the Apache service back up with:

        sudo systemctl start apache2

1. To disable the Apache service, preventing it from beginning automatically at system startup, use:

        sudo systemctl disable apache2

    You can enable the Apache service again using:

        sudo systemctl enable apache2

1. Restart the Apache service with:

        sudo systemctl restart apache2

1. To reload Apache's configuration files, use:
...
1. To search for available modules, use:
...
1. You can subsequently disable a module using:

        sudo a2dismod php7.4

1. To list the modules currently enabled, use:
...
1. Apache comes with a default site configuration. Since this guide is setting up a custom configuration, you need to disable the default:
...
<VirtualHost *:80>
    ...
    This sets up a name-based virtual host for the `example.com` domain. You can host additional domain names by adding a configuration files for each.
...
1. Give the website some content. Create a page for your website. The file should be named `index.html` and stored in your website's `DocumentRoot` directory:
    ...
    {{< file "/var/www/example.com/public/index.html" html >}}
<!doctype html>
<html>
<body>
    <h1>Hello, World!</h1>
