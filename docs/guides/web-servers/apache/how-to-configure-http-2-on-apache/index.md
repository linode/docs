...
modified_by:
  name: Linode
title: "How to Configue HTTP/2 on Apache"
h1_title: "Configuring HTTP/2 on Apache."
contributor:
  name: Jeff Novotny
...
For more comprehensive information and a collection of resources about HTTP/2, see Linode's [Introduction to HTTP/2](/docs/guides/an-introduction-to-http-2).
...
1.  Installing Apache.
2.  Installing the Necessary PHP Components.
3.  Configuring Apache to Support HTTP/2.
4.  Enabling HTTPS Support.

### Installing Apache

Run the `apache2 -v` command to determine whether Apache is installed. If it is already present, the command indicates what version is running. In this case, skip this section and proceed to the "Installing the Necessary PHP Components" step. If the command displays an error, Apache is not yet installed. For more information about Apache, see Linode's [Apache configuration guide](/docs/web-servers/apache-tips-and-tricks/apache-configuration-basics).
...
3.  Verify Apache is `active` using `systemctl`.
...
apache2.service - The Apache HTTP Server
    ...
    {{< /output >}}
4.  **Optional** To configure Apache so it does not launch automatically whenever the server reboots, disable the entry in `systemctl`. To configure it to activate on reboot again, use the `systemctl enable` command.

        systemctl disable apache2
5.  Configure the `ufw` firewall to enhance security. Allow `ufw` to accept `OpenSSH` and `Apache Full` connections. `Apache Full` allows both HTTP and HTTPS requests through.
...
7.  Enable `ufw`.
        ...
        sudo ufw enable
8.  Confirm the web server is working. Type the address of the Linode server into the address bar of the browser. If Apache is working properly, the browser displays the default Apache landing page. The connection is still using HTTP/1.1 at this point.

### Installing the Necessary PHP Components
...
If PHP was not previously installed, the message `ERROR: Module php7.4 does not exist!` is displayed when `php7.4` is disabled. You can ignore this message.
    {{< /note >}}
2.  Enable the `proxy_fcgi` module.

        sudo a2enmod proxy_fcgi

### Configuring Apache to Support HTTP/2
...
6.  Verify Apache is still `active` using the `systemctl status` command.
        ...
        systemctl status apache2
7.  Locate the `.conf` file that contains the `Protocols` definition. Depending on the installation, this is either found at `/etc/apache2/apache2.conf` or `/etc/apache2/mods-available/http2.conf`. To find the exact file, change to the root directory for Apache and search for the `Protocols` keyword with `grep`. Select the file that already contains a `Protocols` configuration. If there are no matches, choose the base `apache2.conf` file.
...
Earlier versions of Apache have a different file and directory structure. The main Apache `.conf` file might be located at `/etc/httpd/httpd.conf`, `/etc/httpd/conf/httpd.conf`, or `/usr/local/apache2/apache2.conf`.
    {{< /note >}}
8.  Edit this file and add the `h2` and `h2c` protocols to the `Protocols` definition. Apache tries to negotiate the protocols in the order they are listed. Placing `h2` first prioritizes HTTP/2 over HTTP/1.1.
    ...
    {{< /file >}}
    {{< note >}}
HTTP/2 support is typically configured on a system-wide basis. If you only want to enable HTTP/2 for one site, add the `h2 h2c` protocols to the virtual server entry for the site instead. See the [Linode Apache configuration guide](/docs/web-servers/apache-tips-and-tricks/apache-configuration-basics).
    {{< /note >}}

### Enabling HTTPS Support

Most clients only support HTTP/2 if encryption is enabled, so HTTPS must be enabled to use it. HTTPS allows for website authentication and ensures all data is transmitted privately. To accept HTTPS requests, a web site must possess a public key certificate signed by a trusted certificate authority. This certificate ensures the owner actually operates the web site in question.
...
If Snap is not already installed, install it using the command `sudo apt install snapd`.
    {{< /note >}}
2.  Remove any existing Certbot packages to avoid possible conflicts.
...
certbot 1.12.0 from Certbot Project (certbot-eff) installed
    {{< /output >}}
4.  Configure a symbolic link to the Certbot directory.

        sudo ln -s /snap/bin/certbot /usr/bin/certbot
5.  Run Certbot to generate certificates for each domain. You can create multiple certificates with one command by specifying the `-d` option in front of each domain. Substitute your own domain name throughout the following section.
...
## Verifying HTTP/2 Support is Enabled on Apache.

To confirm Apache is using HTTP/2, visit the web site using any browser, and use the developer tools to inspect the incoming packets. The following instructions demonstrate this technique on Firefox. Each browser has its own method of inspecting incoming packets. Consult the browser's documentation for more details.

1.  Visit the domain using Firefox.
2.  Open the Firefox Developer Tools. Select **Tools** followed by **Browser Tools** and then **Web Developer Tools**. This opens a new panel at the bottom of the browser.
3.  Select the **Network** tab, and reload the page again.
4.  This displays a list of several rows. Click on the row corresponding to the base domain. This reveals a new panel on the bottom right. The **Headers** tab appears by default.

    [![Developer panel in Firefox](developer-panel_small.png)](developer-panel.png)
5.  If HTTP/2 is working, the `Status` reads `OK` and the version is `HTTP/2`.
