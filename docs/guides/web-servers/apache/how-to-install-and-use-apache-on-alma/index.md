---
slug: how-to-install-and-use-apache-on-alma
author:
...
modified_by:
  name: Linode
title: "Installing and Using Apache on AlmaLinux"
...
*   It is based on the CloudLinux OS, a RHEL fork that has existed for over ten years. AlmaLinux is a 1:1 binary fork of RHEL.
...
## Installing and Enabling the Apache Web Server on AlmaLinux
...
## Configuring the AlmaLinux Firewall to Allow Apache Connections
...
{{< note >}}
It is strongly recommended that you keep the firewall turned on. To temporarily disable the firewall to debug a problem, enter the command `sudo systemctl stop firewalld`. The firewall can be restarted using the `sudo systemctl start firewalld` command. To disable it permanently, use the `sudo systemctl disable firewalld` command.
{{< /note >}}
...
{{< note >}}
In addition to `firewalld`, the security application SELinux comes pre-loaded with AlmaLinux. Under certain conditions, the two applications might conflict with one another. Verify the status of SELinux using the `sestatus` command. SELinux can be temporarily disabled for debug purposes using the `sudo setenforce 0` command.
{{< /note >}}

## Testing the Apache Web Server and Hosting a Web Page
...
2.  The location for the default web page for the server is `/var/www/html`. If this directory is empty, Apache displays the test page at `/etc/httpd/conf.d/welcome.conf`. Copy the home page for your website, in this case `index.html`, into the `/var/www/html` directory.
...
5.  **(Optional)** You should ideally create a virtual host for each site on the server. For more information on creating virtual hosts on Apache, consult the Linode guide on [Apache Configuration Basics](docs/apache-tips-and-tricks/apache-configuration-basics/).

## Learning More About AlmaLinux
