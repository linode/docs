...
This guide will show you several different NGINX server configurations.
...
In the http block above there’s an include directive that tells NGINX where website configuration files are located. It changes depending upon your source of NGINX installation:

  1. Installation from official NGINX repository: include directive will be `include /etc/nginx/conf.d/*.conf;`. Every website you host with NGINX will get it’s own configuration file in `/etc/nginx/conf.d/`, with names formatted as /etc/nginx/conf.d .
...
Regardless of the installation source, server configuration files will contain a server block(s) for a  website. Here’s an example server block:
...
Here are some examples for server_name NGINX configuration based on sites you want to host on your server.
...
The locations above are literal string matches, which means that a request to `http://example.com/planet/blog/` or `http://example.com/planet/blog/about/` will be fulfilled by `location /planet/blog/` , even though `location /planet/`  also matches this request.

## Configuring NGINX location directive using regex match

When a  `location`  directive is followed by a tilde (~), NGINX server performs a regex (or regular expression) match. NGINX uses [Perl Compatible Regular Expression (PCRE)](https://perldoc.perl.org/perlre) for regex. Here’s an example:
...
To get started with configuring a reverse proxy, follow these steps. 
  ...
  5. Use the symbolic link and copy configuration from `/etc/nginx/sites-available` to `/etc/nginx/sites-enabled` by
    `ln -s /etc/nginx/sites-available/reverse-proxy.conf /etc/nginx/sites-enabled/reverse-proxy.conf`

  6. Check if NGINX is working by
    `nginx -t`
