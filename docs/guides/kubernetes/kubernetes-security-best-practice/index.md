...
The primary purpose of provisioning a Kubernetes cluster is to deploy an application. Securing workloads in Kubernetes begins before the workloads are deployed within the cluster; it begins when creating an application image in the CI/CD pipeline; it is vital to create a secure image since it is the first step in securing your cluster. To reduce the possibility of viruses or backdoors, it is better to avoid utilizing codes from untrusted registries or libraries while building your application image. Furthermore, avoid utilizing unverified operating system packages in your container image because they can include backdoors. After you've built your image, scan it for vulnerabilities. There are many tools available to help you with image scanning, such as [Snyk](https://snyk.io/) and [sysdig](https://sysdig.com/). Regularly scan your image with these scanning tools because they maintain a database of known vulnerabilities that are updated on a regular basis. You can archive it by incorporating these tools into your CI/CD setup.

Even after scanning for vulnerabilities in the CI/CD pipeline, some vulnerabilities can be missed because it is possible for new vulnerabilities to be discovered after you have scanned your image. Many image registries, such as Docker image registry, have an image scanning function built in. So, make sure to always scan images in the registry.

## Avoid Running your Container as a Root user or with Privileges 

If you have a vulnerability in your image and the container is operating with privileges, an attacker can easily exploit it to break out of your container and gain access to the host or the Kubernetes work node. As a result, while creating your image, you should establish a service user and run the application with that user rather than the root user. 
