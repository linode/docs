...
og_description: "Looking to remove resources from your Docker instance? Look no further than our guide on how to remove Docker images, containers, and volumes!"
...
h1_title: "How to Remove Docker Images, Containers, and Volumes"
contributor:
  name: Nathaniel Stickman
...
Docker containerization can make working with applications and services easier, however complex they may be. But you may want to periodically manage your accumulated Docker resources. Docker takes a conservative approach to "garbage collection," which can leave you with unused Docker resources sitting around, especially on development environments.
...
1. Follow the steps in our [Installing and Using Docker](/docs/guides/installing-and-using-docker-on-ubuntu-and-debian/) guide to install, setup, and run a Docker instance. You can use the drop down at the top of the page to select the Linux distribution matching you system.
...
- To remove all unused containers and networks and all dangling images:

        docker system prune

- To remove all unused images instead of just dangling ones:

        docker system prune -a

- To expand the above command to additionally remove all unused volumes:
...
It can be helpful here to get the list of images currently on your Docker instance. Especially so since some of these commands use image names and/or IDs. You can get a list of images with this command:

    docker image ls -a

### Removing Images by Name

To remove an individual image based on its ID, use a command like the one below. Replace `feb5d9fea6a5` with the ID of the image you want removed:

    docker rmi feb5d9fea6a5

This command does not work for images associated with a container, even when that container that has been stopped.
...
The command above does not in itself fully remove the image. To ensure that the image is fully remove, you need to remove all tags associated with it. So, if you have the above image tagged with `anotherTag` as well, you need to run the `rmi` command again for `hello-world:anotherTag`.
...
Images that are not tagged and are not associated with at least one container are called *dangling* images. You can get a list of these images using:
...
Images that are not associated with a container but may or may not be tagged are considered *unused* rather than dangling. You can extend on the command above to remove these images as well:

    docker image prune -a

### Removing All Images

Docker does not have a built-in command for removing all images. Instead, you can use the `$` command-line operator to feed Docker a list of images:
...
This section shows you how to remove containers from your Docker instance. It gives you the commands for removing containers by ID or name, for removing stopped containers, and even for removing all containers for your instance.
...
The above command does not, however, include stopped containers. To get a list that includes stopped containers, use:
...
As with the `docker rmi` command covered in the image section above, this command support multiple arguments:

    docker rm 8e7fc32ab606 f56b33a151f2

### Removing Stopped Containers

Docker comes with a convenient command for removing all stopped containers:
...
You can remove all containers using the same method as above, with the `$` command-line operator:
...
In contrast to images, volumes are considered dangling simply when they are not referenced by any container. You can get a list of current dangling volumes using:
...
In this tutorial, you have everything you need to delete unused and unwanted resources from your Docker instance. Using these tools, you can more effectively manage your Docker resources and keep dangling resources from piling up.
