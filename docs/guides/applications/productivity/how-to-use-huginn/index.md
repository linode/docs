...
It is possible to manually install Huginn. However, the Docker method is used here since Huginn does not support the latest Debian and Ubuntu releases for the manual installation method. However, Huginn maintains [manual installation instructions](https://github.com/huginn/huginn/tree/master/doc/manual) if you prefer that installation path. Huginn requires that your server be running Debian 8 or 9 or Ubuntu 14.04, 16.04, or 18.04 for the manual installation process.
...
Agents come in a few different roles. Broadly, agents are either event sources, event receivers, or a combination.
