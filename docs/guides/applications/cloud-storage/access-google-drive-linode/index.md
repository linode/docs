...
description: 'Access Google Drive from your Linode with Ubuntu 14.04 using google-drive-ocamlfuse to connect directly with the Google Drive API.'
...
First, we will add the repository where google-drive-ocamlfuse lives in our Linode. Once that's done, we update so the changes are visible to us and then install normally.
...
## Authorize google-drive-ocamlfuse Access

Next, we'll provide the credentials for your Drive API to google-drive-ocamlfuse in your Linode, authorizing it to access your Google Drive.
...
2.  Google will ask for permission to allow this new application (google-drive-ocamlfuse) to access your Google Drive. Click **Accept** to receive the verification code:

    ![The permission request screen from google.](google_authorization.png)

3.  Copy/paste the verification code back to google-drive-ocamlfuse in your Linode.
...
5.  Repeat the steps to [authorize google-drive-ocamlfuse access on your Linode](#authorize-ocamlfuse-access).
