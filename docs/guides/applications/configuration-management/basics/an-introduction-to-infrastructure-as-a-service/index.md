---
slug: an-introduction-to-infrastructure-as-a-service
author:
  name: Linode Community
  email: docs@linode.com
description: 'Infrastructure as a Service (IaaS) is a cloud computing service that provides virtual infrastructure to customers on demand. This guide describes IaaS and explains its advantages and rationale, and outlines how to use it.'
og_description: 'Infrastructure as a Service (IaaS) is a cloud computing service that provides virtual infrastructure to customers on demand. This guide describes IaaS and explains its advantages and rationale, and outlines how to use it.'
...
Customers can access their virtualized infrastructure over the internet. They can use a visual dashboard or graphical user interface to quickly create or modify devices, often with the push of a button. The dashboard can also be used to monitor performance, collect data, troubleshoot, and track costs. All services are provided on a pay-as-you-go model. Some organizations might develop their own private cloud rather than using a provider, but this model is typically only used by large technology companies.
...
One important point about IaaS is the customer does not control the underlying physical hardware components and interconnections. These remain under the control of the cloud provider. But clients are responsible for the selection and installation of the operating system and all software applications, including databases and middleware.
...
*   **Bare-metal-as-a-service (BMaaS):** This service provides users with direct access to the hardware, except the provider owns the devices. It is very similar to a straight rental service. BMaaS gives customers more control over the architecture and layout of their network. Clients should find this very similar to running their own lab, without the operational costs or capital expenses. However, BMaaS provides no further advantages over an in-house lab, and none of the IaaS optimizations.
...
*   **SaaS:** SaaS provides software services to customers on demand. Users access the software on the provider's server, typically through a web browser. SaaS clients are only responsible for configuring the application, maintaining an access control list, and the actual content. SaaS is usually geared towards a completely different audience. Most SaaS clients do not require a platform and have no need for IaaS capabilities.
*   **Serverless:** Serverless computing eliminates the necessity of infrastructure management. It lies somewhere between PaaS and SaaS in the control/ease-of-use scale but is not exactly like either model. In fact, the name "serverless" is somewhat misleading. There are servers being used, but the end user has no knowledge or visibility of them. Serverless computing provisions all resources on demand, and automatically and dynamically scales them up and down in close to real time. This makes more efficient use of computer resources because the provider allocates the memory, cpu and networking resources based on the calculated demand. Serverless computing is well suited to a microservices model, and is frequently used in software development. However, it can add more latency and cold starts can affect the system performance.
...
Although efficiency and ease of use prompts most deployments, the IaaS model is particularly well suited to the following scenarios.

*   **Testing and rapid development:**. IaaS allows for quick prototyping and for efficient automated testing. Servers can be created and deleted as required. IaaS facilitates testing of an application on different platforms and networks. It is also useful for temporary or experimental work flows.
...
*   **High-performance and specialized computing:** The cost of buying high-performance equipment capable of specialized tasks might otherwise be prohibitive for smaller firms. IaaS enables smaller firms to access advanced systems capable of handling data analysis, computer modelling, and 3-D graphics.
...
*   It reduces maintenance, operating costs, and lab space. IaaS allows businesses to focus on their core activities instead of running a lab.
...
*   IaaS providers offer different packages with different levels of memory and performance. It is easy for a customer to find the right package for their network. Customers can also upgrade or downgrade according to their current situation.
...
There are very few disadvantages to IaaS platforms. The biggest drawback is the relative lack of flexibility and low-level visibility compared to an on-site lab. Customers cannot deploy a system that their IaaS vendor does not offer, and they cannot control attributes such as IP address blocks and subnets. However, this is usually not a big concern for most deployments.
...
Several options are available to organizations that want to relocate their existing network to use IaaS techniques.
...
Additionally, a company might reduce or retire certain legacy systems. This is especially likely to happen during a refactoring process. However, any detailed planning process should seek to identify unnecessary items. Occasionally, a finished prototype or experimental product could be migrated the other way, from an IaaS network back to the on-premise lab.
...
*   Consider how any existing databases and servers should be migrated using one of the techniques in the "Migration Strategies" section.
*   Attempt to reduce downtime. Schedule a maintenance window for the migration.
*   Test the new network before live deployment.
*   Consider how much storage and what storage types should be used. The main types of storage are object storage, file storage, and block storage. Object storage has become more popular recently because its distributed architecture fits well with the IaaS model.
