...
If Terraform does not find a default value for a defined variable; or a value from a `.tfvars` file, environment variable, or CLI flag; it prompts you for a value before running an action:
...
-   [git-crypt](https://github.com/AGWA/git-crypt) allows you to encrypt files when they are committed to a Git repository. git-crypt also decrypts files when they are checked out.
    ...
    {{< note >}}
You must initialize git-crypt in a repository before committing your state file or variable value files, else the files are not eligible for encryption.
{{< /note >}}
...
If you are unwilling or unable to use the above options to help manage the state file, and if you are using a platform like GitHub or GitLab to share your state files, then at minimum the repository should be private.
...
You can also set secrets directly in your environment variables. And you can define environmental variables that are automatically picked up everytime you run Terraform.
...
{{< note >}}
After the variables are properly defined, the next time you run Terraform, it automatically picksup secrets
terraform apply
{{< /note >}}
...
You can also use a secret store for Terraform secret management. Using a secret management store like HashiCorp Vault which is open source and cross-platform helps you store sensitive data and limits who can access it.
...
You now can manage secretgs with the Terraform code.
