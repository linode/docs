...
- **Migration of Compute Instances:** Once a data center has started the network infrastructure upgrades, live migrations are scheduled for all Compute Instances that do not reside on upgraded hardware. This live migration will occur while your Compute Instance is powered on and operating normally. After the migration has been successfully completed there may be only a small amount of packet loss during the cutover point. The Compute instances are not rebooted during live migrations. For Compute Instances where live migrations are not possible due to lack of compatible destination hardware, cold migrations are performed. This involves a downtime and the customer support will notify you about the downtime.
