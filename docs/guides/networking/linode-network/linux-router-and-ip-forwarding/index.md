...
1.  Log in to each Linux system that is *not* designated as the router. To do so, you can use [SSH](/docs/guides/connect-to-server-over-ssh/) from the router, or [Lish](https://techdocs.akamai.com/cloud-computing/docs/access-your-system-console-using-lish) if using an Akamai Cloud Compute Instance.
