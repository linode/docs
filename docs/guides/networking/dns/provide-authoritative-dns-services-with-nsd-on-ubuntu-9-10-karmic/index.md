...
Test the configuration and functionality of the DNS serve using `dig`, which provides a [command line DNS client](/cloud/guides/use-dig-to-perform-manual-dns-queries/). If `dig` is not installed, install the utility by issuing the following command:
