...
Ask an IT manager which networking type is best, and you’re likely to get the classic engineering response: “It depends.”
...
This is a good place to start, because the designs used to tie networks together also apply within a single network. Most networks use one of three basic designs: Point to point, hub and spoke, or meshed.

A point-to-point connection is just a link between two networks or computers.

A hub-and-spoke design connects multiple end-stations (computers, printers, IoT devices) to a central server; for example many PCs using a single server for file storage.

Meshed networks have two variants, partial and full. In a fully meshed design, all nodes (servers, routers, switches) connect to all other nodes. This provides the greatest amount of redundancy in case of link or node failures, but also the greatest cost if you’re paying for each link. In a partial mesh, only some nodes are interconnected. The global Internet is a partially meshed network.

The basic building block of the Internet and many internets is the TCP/IP protocol suite, a set of open standards that allow all manufacturers’ devices to interoperate.

Suitable for:
Connectivity between any set of devices, regardless of manufacturer

Not suitable for:
Very old legacy computers that use proprietary communications protocols
...
Today the meaning of LAN is much broader: You may have a wired LAN in your office, or at home. And you may connect a much broader range of devices to LANs, including surveillance cameras, Internet of Things (IoT) devices, and even solar power controllers in home networks.
...
Not suitable for:
Computers across town or across the world
...
Single-mode fiber tends to be more expensive than copper, mainly due to transceiver and fiber termination costs. And while POLANs can be an excellent option in new deployments, you’ll need to factor in the cost of ripping out copper cabling when evaluating them as a replacement technology.
