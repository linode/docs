---
slug: how-to-use-nfs-server-on-ubuntu
author:
  name: Linode Community
  email: docs@linode.com
description: 'Network File System (NFS) is a distributed file system that allows users to access files over a network like they would local files. This guide provides a brief introduction to NFS and explains how to configure and use NFS on both server and client systems.'
og_description: 'Network File System (NFS) is a distributed file system that allows users to access files over a network like they would local files. This guide provides a brief introduction to NFS and explains how to configure and use NFS on both server and client systems.'
...
*Network File System* (NFS) is a distributed file system that allows users to access files over a network like files on their own system. The current version of NFS, which was originally developed by Sun Microsystems, is [*Version 4*](https://datatracker.ietf.org/doc/html/rfc7862). NFS is an open standard that builds on the *Open Network Computing Remote Procedure Call* (ONC RPC) system. This guide provides a brief introduction to NFS and explains how to configure and use NFS on both server and client systems.
...
*   It is easy to set up and easy for both administrators and clients to use. NFS has a low administrative overhead after initial configuration has been completed.
...
2.  This guide uses `sudo` wherever possible. Complete the sections of Linode's guide on [Securing Your Server](/docs/security/securing-your-server/) to create a standard user account, harden SSH access and remove unnecessary network services. Do **not** follow the Configure a Firewall section yet. This guide includes firewall rules specifically for an OpenVPN server.

3.  Update your system:

        sudo apt-get update && sudo apt-get upgrade

4. To complete the server and client configuration, two Linodes are required. One Linode serves as the NFS host and server, while the other acts as a client. Note the IP addresses of both Linodes. Throughout the following sections, replace `server_ip_addr` with the IP address of the NFS server, and `client_ip_addr` with the address of the client.

{{< note >}}
This guide is written for a non-root user. Commands that require elevated privileges are prefixed with `sudo`. If you are not familiar with the `sudo` command, see the [Users and Groups](/docs/tools-reference/linux-users-and-groups/) guide.
{{< /note >}}

## Configuring the NFS Server and Client

The steps involved in configuring the NFS server and the client are as follows:

1.  Installing the NFS Server and Client Software.
2.  Configuring the Firewall on the NFS Server.
3.  Creating an Export Directory on the NFS Server.
4.  Configuring the NFS Export Settings on the Server.
5.  Creating an NFS Mount Point on the Client.

### Installing the NFS Server and Client Software
...
1.  On the NFS server, update all packages.
        ...
        sudo apt update
2.  Install the `nfs-kernel-server` package on the server.
...
By default, the NFS server supports versions 3 and 4. Verify the versions that are enabled using the `sudo cat /proc/fs/nfsd/versions` command.
    {{< /note >}}
3.  Verify the status of the NFS server using the `systemctl` utility. It should display a status of `active`.
...
nfs-server.service - NFS server and services
   ...
   Main PID: 2588 (code=exited, status=0/SUCCESS)
    ...
    {{< /output >}}
4.  Update all packages on the Linode serving as the client.

        sudo apt update
5.  On the client, install the `nfs-common` package.

        sudo apt install nfs-common

### Configuring the Firewall on the NFS Server
...
Apache Full (v6)           ALLOW       Anywhere (v6)
    {{< /output >}}
2.  Add a rule to allow port `2049` to accept traffic from the client's IP address. Replace `client_ip_addr` with the address of the client Linode.

        sudo ufw allow from client_ip_addr to any port nfs
3.  Run `ufw status` again and confirm the rule has been added.
...
### Creating an Export Directory on the NFS Server
...
2.  For security reasons, NFS translates any root credentials from a client to `nobody:nogroup`. This restricts the ability of remote root users to invoke root privileges on the server. To grant client root users access to the export directory, change the directory ownership to `nobody:nogroup`.

        sudo chown nobody:nogroup /nfs/share/project1
3.  Adjust the file permissions for the directory. The following command grants NFS clients full read, write, and execute privileges to the directory files.

        sudo chmod 777 /nfs/share/project1

### Configuring the NFS Export Settings on the Server

The NFS component maintains a configuration file containing the access control lists for each export directory. Each entry indicates the name of the export directory, the clients who are authorized to access it, and the file sharing options. The format of each line is `export_directory_name client1(sharing_options) client2(sharing_options_2)`. Access can be provided to several clients in the same entry. A client can be identified either through a specific IP address, an entire subnet, or a domain name.
...
1.  Edit the `etc/exports` file.
        ...
        vi etc/exports
2.  Add the following line and save the file. Replace `client_ip_addr` with the actual IP address or subnet of the client.
    {{< file "/etc/exports" aconf >}}
/nfs/share/project1  client_ip_addr(rw,sync,no_subtree_check)
    {{< /file >}}
3.  Apply the configuration changes using the `exportfs` command.

        sudo exportfs -a
4.  View all the active exports and verify the `/nfs/share/project1` directory has export status.
...
client_ip_addr(rw,wdelay,root_squash,no_subtree_check,sec=sys,rw,secure,root_squash,no_all_squash)
    {{< /output >}}
5.  Restart the NFS utility using `systemctl`.

        sudo systemctl restart nfs-kernel-server

### Creating an NFS Mount Point on the Client
...
{{< caution >}}
Only use a completely empty directory for the mount point. Any pre-existing files or subdirectories in the mounted directory are hidden while the mount point is in use.
{{< /caution >}}

1.  Create a new directory on the client to serve as the mount point.

        sudo mkdir -p /nfs/mnt/project1
2.  Mount the export directory to the new directory using the `mount` command. As the first argument, specify the IP address of the NFS server, a `:` symbol, and the path to the export directory. The second parameter is the local mount point directory on the client.

        sudo mount -t nfs  server_ip_addr:/nfs/share/project1  /nfs/mnt/project1
3.  Use the `df` command to confirm the export directory is mounted correctly. The remote directory should be listed somewhere in the list of drives.
...
1.  Log back in to the server and create a new file within the export directory.

        cd /nfs/share/project1
        "Test data for testfile.text" > testfile.text
2.  Change to a console on the client, and `cd` to the mounted directory. Run the `ls` command, and ensure the `testfile.text` file appears.
...
-rw-rw-r-- 1 user1 user1 27 Jul  7 13:06 testfile.text
    {{< /output >}}
3.  Remain on the client, and use a text editor to add some additional text to the `testfile.text` file.

        vi testfile.text
4.  Return to the server console and ensure the `testfile.text` file in the export directory reflects the changes that were made on the client.
...
## Unmounting an NFS Share on the Client
...
2.  Use the `df` command to confirm the mount point has been removed.

        df -h
3.  Edit the `/etc/fstab` file and either remove the line referring to the export directory or comment it out with the `#` symbol.
