...
5.  Allow outgoing connections, and then drop all other packets:
    ...
    {{< note >}}
If you've configured your `iptables` firewall by following the [Securing Your Server](/docs/guides/securing-your-server/) tutorial, then you need to append the exceptions in steps 1, 2 and 3 to `/etc/iptables.firewall.rules` to ensure that they're persistent between reboots.
{{< /note >}}
