...
modified_by:
  name: Linode
title: "Learn How to Create a Swap File in Ubuntu | Linode"
...
Swap space is located on the system's storage drive. As of release 17, Ubuntu uses a swap file rather than as a partition. A swap file is a pre-allocated file that is configured to hold system swap data. A swap file is easier to resize than a partition when more swap space is required. On a virtual machine, where partitions are not available, a swap file is the only available option.

Even though the swap file is located on the storage drive, the system can use this space as if it were part of RAM. Active program data can be temporarily stored in either place. This process is transparent to the user and it is not possible to tell where any given piece of data is being stored.
...
*   It can cause excessive disk wear-and-tear on older SSD drives. This is because working memory is accessed far more frequently than the permanent storage drive. Newer SSD drives do not typically have this problem.
...
These instructions apply to Ubuntu release 20.04 and higher. However, the same basic approach has been used since release 17. On older releases, it is necessary to create a swap partition to add swap space on Ubuntu. These instructions are only valid for creating a swap file on Ubuntu and are not necessarily accurate on other Linux distributions.

It is important to configure a large enough swap file. However, very large files are typically not necessary. A system with 2GB of RAM or less should have a swap file that is twice as large as the RAM. For larger systems, 4GB of RAM should suffice. The optimal amount of swap space depends on the system and how it is used.

It is possible to create multiple swap files. However, it is better to configure one swap file, especially because it is possible to resize it later.
...
2.  Confirm swap space has not already been allocated using the `free` command. This command can be used on most Linux systems to verify the swap space size. If the total memory for the `Swap` memory is `0B`, a swap file has not been created yet.
...
6.  Run the `ls` command to confirm the swap file has been successfully created. Use the `-l` option to see the size of the file.
        ...
        ls -hl /swapfile
    {{< output >}}
-rw------- 1 root root 1.0G Feb  9 12:11 /swapfile
    {{< /output >}}
...
no label, UUID=16ea9e60-2bee-4dee-9181-e08b79336d46
    {{< /output >}}
...
10. To confirm the amount of swap space available on Ubuntu, use the `free` command. The `total` column for the `Swap` entry should display `1.0Gi` of total memory.

        free -h
    {{< output >}}
              total        used        free      shared  buff/cache   available
Mem:          3.8Gi       523Mi       2.5Gi       3.0Mi       833Mi       3.1Gi
Swap:         1.0Gi          0B       1.0Gi
    {{< /output >}}
...
One advantage of using a swap file over the old partitioning method is it is easy to resize an existing swap. On an Ubuntu system, follow these steps to add swap space.
...
*   Set `bs` to `1G`. This stands for "block size",
...
2+0 records in
2+0 records out
2147483648 bytes (2.1 GB, 2.0 GiB) copied, 10.3904 s, 207 MB/s
    {{< /output >}}
...
no label, UUID=8091086d-dbc4-4ac8-8f86-d7034db0f9a5
    {{< /output >}}
...
6.  Confirm the file has been resized correctly and is ready for use using either the `free` or `swapon --show` command.
    ...
    {{< output >}}
              total        used        free      shared  buff/cache   available
Mem:          3.8Gi       531Mi       1.1Gi       3.0Mi       2.2Gi       3.1Gi
Swap:         2.0Gi          0B       2.0Gi
    {{< /output >}}
