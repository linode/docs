...
- It can copy a file to the same directory. The new file must have a different name.
- It can copy a file to a different directory. It is possible to rename the file or retain the old name.
- It can be used to copy multiple files to a different target directory.
- It can be used to recursively copy the contents of a directory, including subdirectories, to a different target directory.
...
- **-p**: Preserves the file attributes of the original file in the copy. File attributes include the date stamps for file creation and last modification, and the user ID, group IP, and file permissions.
...
The options `-H`, `-L`, and `-P` indicate how the `cp` command should process symbolic links. See the [cp man page](https://pubs.opengroup.org/onlinepubs/9699919799/utilities/cp.html) for a full description of `cp` and symbolic links. The options for `cp` vary between Linux distributions. For a list for Ubuntu 22.04, see the [Ubuntu cp documentation](https://manpages.ubuntu.com/manpages/jammy/man1/cp.1.html).
