...
To install a standard VSFPTD on Ubuntu 20.04 through the command line follow these steps:
...
1. Finally, ask FTP for the contents of the file:
...
This is the content in the file.
    ...
    At its most basic, you now see how VSFTPD works.  You can also establish network connections, add more sophisticated account management, encryption, and security restrictions, as the next sections illustrate.
...
In Ubuntu and other common distributions, VSFTPD's configuration appears is located in the following location: `/etc/vsftpd.conf`.
...
Start by blocking anonymous access, enabling authentication through your server's account system, enabling uploads, and restricting access to the local user's `$HOME`:
...
Update `/etc/vsftpd.conf` so that the specific attributes shown above have the specified `YES` and `NO` values, then system restart VSFTPD as explained above.
...
1. Ubuntu's default built-in firewall blocks FTP traffic.  Open the firewall to allow desired FTP traffic by creating an exception in UFW (uncomplicated firewall) with the commands:
...
The list below contains some additional configurations you may consider depending on how you will use your VSFTPD server:
