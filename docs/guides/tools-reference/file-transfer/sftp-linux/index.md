...
Remote working directory: /home/userid
    {{< /output >}}
1. File navigation works much as it does in a normal Linux context. Use the `cd` command to change to a new directory. To navigate relatively, follow `cd` with the name of the target directory. The `cd ..` command moves the context up one level.
