...
description: 'A guide about how to find files in Linux using the command-line interface using the find command. This will help you learn how to find image files, files with specific text, files in the current directory, and much more.'
...
When you have to find a Linux file, it's sometimes not as easy as finding a file in another operating system, especially if you are running Linux without a graphical interface and need to rely on the *command-line interface* (CLI). This article covers the basics of how to find a file in Linux using the CLI. The `find` command in Linux is used to find a file (or files) by recursively filtering objects in the file system based on a simple conditional mechanism, and that's what you should use to search for a file or directory on your file system. By using the `-exec` flag (`find -exec`), files can be found and immediately [processed within the same command](#how-to-find-and-process-a-file-in-linux).
