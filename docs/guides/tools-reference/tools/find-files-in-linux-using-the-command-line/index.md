...
The `find` command can only filter the directory hierarchy based on a file's name and metadata. If you need to search based on the file's content, use a tool like [`grep`](/cloud/guides/how-to-use-grep-command/). Consider the following example:
