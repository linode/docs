...
`broot` is a modern approach to directory trees in the command line. It renders the tree in a brief and intuitive layout that makes getting around and getting the gist quicker and easier. Also, `broot` comes with advanced features for getting details about and managing your files and directories, all from within one tool.
...
These and the following examples run `broot` in a cloned Git repository as an example. This makes it easier to show further how `broot` integrates with Git. You can get the repository used here by running the following commands in your user's home directory. Replace `apt` with `yum` on **AlmaLinux** and **CentOS** or with `dnf` on **Fedora**.

    sudo apt install git
    git clone https://github.com/google/docsy-example.git

Now, you can open `broot` using the `br` command followed by the repository's directory.
...
- Use the combination of **Ctrl** and the right arrow key to open a separate panel for the selected file or directory.
    ...
    If you used the **Ctrl** and the right arrow key combination on a directory, the new panel gives a directory tree with its root in the selected directory.
...
- Similar to the shell's `ls` command, `broot` is capable of giving a high level of detail about the files and directories it displays. Following is an example with results similar to the `ls -al`.

    Use the `:dates` command to show the last modified dates, then the `:perm` command to show permission information. Finally, use `:h` to show hidden files. The result should look something like the following:
...
One of the unique setting in `broot` is its file management capabilities. You get a suite of file-management options right along with the easy-to-read directory tree. Following are some examples that can cover most day-to-day file-management needs.
...
`broot` has many more features and capabilities to explore. Like, `broot` is able to provide you with more search options, the ability to export directory trees, or with some `broot` options at launch.

You can keep going with some of the basics of `broot` by referring to [their GitHub readme](https://github.com/Canop/broot).

Go deeper and start learning some of the advanced features of `broot` by referring to the [broot official documentation](https://dystroy.org/broot/), which is thorough. It also helps you navigate the furthest reaches of `broot`.
