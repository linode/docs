...
keywords: ['bottom linux','bottom linux install','system monitor linux terminal']
...
`bottom` is a graphical system monitoring dashboard for the terminal. What sets it apart from similar tools is the high degree of customization options and features for exploration that `bottom` gives you. Whether you want to get a quick graphical summary of your system or to dive through its running processes, `bottom` has your needs covered.
...
In short, prefer `bottom` if you want a graphical system monitoring dashboard for the terminal that gives you control of every details of the display. `gtop`, on the other hand, should be your tool of choice if you want uncomplicated system monitoring and find the default display useful.

You can learn more about `gtop`, and more details about how it compares to `top` and `htop`, in our guide [How to Install and Use gtop](/docs/guides/how-to-use-gtop).

## How to Install bottom

Currently, `bottom` is not available in the default package managers for most major Linux distributions. However, it is still easy and straightforward to get `bottom` installed on your system. Just follow the instructions in the section for your Linux distribution below.
...
Check the `bottom` [releases page](https://github.com/ClementTsang/bottom/releases), and replace the version number above (`0.6.3`) with the latest you find there.
...
You can start up `bottom` with the following command. `bottom` opens with an array of widgets for you to immediately start monitoring your system's performance:

    btm

[![Default display in bottom](bottom-display_small.png)](bottom-display.png)

Keep on to the next sections to learn more about the display, how to navigate it, and how to customize `bottom`.

### Widgets in bottom

By default, `bottom` displays the following widgets:

- The **CPU** widget shows a graph of CPU usage over the past minute. Additionally, the widget includes the current percentage usage for each CPU and an average among them.
- The **Memory** widget graphs memory usage over the past minute and shows current RAM and swap memory usage percentages.
...
- The **Network** widget shows received (**RX**) and transmitted (**TX**) network traffic, both in a graph over the past minute and in a current percentage.
...
- Navigate among the widgets using the **Shift** and arrow keys together. If the widget has a list, you should see an item in its list selected when the widget gets selected. Otherwise, you may not be able to tell that a widget is selected until you use one of the widget-related commands below.
...
- The **e** key allows you to expand a widget's contents to fill the display. You can see an example below with the **Network** widget. Pressing **e** again reduces the widget again.
...
Here is an example of how you could use the command-line flags with `bottom`:

    btm --basic --group --process_command --network_use_bytes --regex

The options in this example run `bottom` with:
...
But the configuration file really shines in allowing you to tightly manage how the display in `bottom` is arranged and themed. Using the appropriate tagging pattern, you can completely control the arrangement of widgets and the coloring in `bottom`.

To help you see how this works, below is the default widget arrangement, which you can find a version of commented out in the default configuration file. This example also includes the brief explanation of each tag's role that is also included in the default configuration file:
