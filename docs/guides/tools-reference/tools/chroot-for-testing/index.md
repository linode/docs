...
On running the `chroot` command against that directory, you are able to use the directory as its own functioning system. And, sense the base of the directory you created acts as the root directory, anything operating inside of the `chroot` environment is restricted to the contents of the `chroot` directory.
...
This section shows some basics for setting up a `chroot` environment for testing. Obviously, you are likely to need additional steps to set up the environment for you specific testing scenarios. However, these basics are meant to cover commonly needed configurations regardless of testing scenario.

1. Run Bash in the `chroot` environment, as shown in the section above, and create a limited user using a command like the one below. This user's username (`example-user` in this example) needs to match the limited user you are using to access the `chroot` environment:

        adduser example-user

    If you require your user to have `sudo` access for the `chroot` testing, use a command like the following to give that access to the user:
...
1. Open the `schroot` configuration file — `/etc/schroot/schroot.conf` — and add a configuration for your `chroot` environment.
    ...
    The file comes with several configuration examples. What follows is a simple example used for this guide. Replace `example-user` with the username of the limited user you plan to access the `chroot` environment through:
...
## Exiting and Removing a chroot Environment
