...
description: "Vi has become one of the most popular text editors on Unix systems thanks to its highly efficient keybindings. In this guide, find out more about what Vi editor is and how to use different Vi editor commands including copy and paste, search and replace text, etc."
og_description: "Vi has become one of the most popular text editors on Unix systems thanks to its highly efficient keybindings. In this guide, find out more about what Vi editor is and how to use different Vi editor commands including copy and paste, search and replace text, etc."
...
But perhaps even more important for Vi's popularity is its reputation for a highly efficient command system. As an editor, Vi has the goal of giving its user a robust editing experience without ever having to leave the keyboard's home row. With a dedicated command mode, Vi utilizes the full keyboard for tasks like navigating a file, manipulating text, searching, and replacing phrases.
...
You can navigate text in Vi using the arrow keys, whether in *Normal* or *Insert* mode. However, you can also use the following keys in *Normal* mode. Some of these keys just help keep you from leaving the home row for the arrow keys, but most add handy and efficient navigation shortcuts:
...
The same motif applies to cutting. Vi's base command for cutting is `d`. So, you can take all of the above commands for copying text and replace `y` with `d` to cut the text instead. In Vi, the cut commands are also generally what you use for deleting text.
...
To search and replace text in a file, use the `:s/` command followed by the search text, the replacement text, and `g` to make the search and replace global. Here is an example that replaces all instances of `TODO` in the file with `DONE`:
