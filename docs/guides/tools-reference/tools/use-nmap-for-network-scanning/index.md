...
[*Nmap*](https://nmap.org/) is a popular, open source network scanning tool. It has a wide array of scanning options and capabilities, making it highly efficient and adaptable. Nmap helps system administers keep tabs on a network's host and port usage, and helps security auditors catch potential vulnerabilities.
...
Host scanning is the process of identifying hosts and their current statuses. One of the most useful host scanning procedures is identifying an IP range and scanning that range to see what hosts are there. This kind of host scanning can help keep track of hosts' statuses and help catch unauthorized hosts on the network.

The steps in this section provides an example of how to run a host scan on a network.
...
1.  First, identify the IP range to scan. You may already have an IP range in mind, for instance, if your network has a range of permanent public IP addresses. If not, you can use your network's subnet mask:
    ...
    {{< note >}}
    Debian and Ubuntu do not come with `ifconfig` preinstalled. Run the following command to install it:

    ```command
    sudo apt install net-tools
    ```

    On Debian, you must run `ifconfig` as `sudo`:
    ...
    In this example, the `netmask` shows that the first three numbers in the network's IP addresses (`10.0.0`) identify the network. The last number varies for each host.
...
Port scanning, Nmap's core feature, provides information about the state of ports on a given host or range of hosts. This can help to evaluate what ports are listening for traffic on a host and provide more details about the host itself.
...
You can also scan a range of IP addresses. The following are some of the ways to do that, using the example IP address above:
...
Nmap provides a ton of options to fine-tune scans to your needs. Get a full list of options simply by running the `nmap` command without specifying addresses or options.

To help you get started, below is a list of some of the most commonly used options:
...
Here, Nmap reports on the accessible port it found. It also gives its evaluation of what service is likely running on that port (OpenSSH) and the OS that the service is running on. Nmap's report ends with a further evaluation of the OS running on the host.
