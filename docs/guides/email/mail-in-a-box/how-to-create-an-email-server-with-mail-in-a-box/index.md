...
Since there should not be a propagation delay when making these changes, you can continue on without waiting. If you'd like to confirm that the changes has been made, you can run the following command, replacing *example.com* with your domain:

        dig example.com NS +trace

You should see your name servers at the end of the output, though you may get a message stating that the operation couldn't obtain the IP address. If you don't see your custom domain names within a few minutes, then there might be an issue with how the custom name servers were registered. Contact your registrar for help resolving any issues.
