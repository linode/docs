...
You may want to consider issuing the command to start the tunnel (`/opt/smtp-tunnel start`) as part of your boot script by including it in your `/etc/rc.local` file, or by creating a `@reboot` [cron job](/cloud/guides/schedule-tasks-with-cron/). If your network configuration utility allows you to establish pre- and post-connection scripts, you may want to instantiate and destroy the tunnel during this process. To destroy the tunnel, issue the following command:
...
- [Host Email with Postfix, Courier and MySQL on Debian 5 (Lenny)](/cloud/email/postfix/email-with-postfix-courier-and-mysql-on-debian-5-lenny/)
- [Host Email with Postfix, Dovecot and MySQL on Debian 5 (Lenny)](/cloud/email/postfix/email-with-postfix-dovecot-and-mysql-on-debian-5-lenny/)
