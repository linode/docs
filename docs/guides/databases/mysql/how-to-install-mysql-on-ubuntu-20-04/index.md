---
slug: how-to-install-mysql-on-ubuntu-20-04
author:
  name: Linode Community
  email: docs@linode.com
description: 'How to Install MySQL Server on Ubuntu 20.04 Linux'
og_description: 'How to Install MySQL Server on Ubuntu 20.04 Linux'
keywords: ['Install mysql ubuntu','Install mysql server ubuntu','Install mysql linux']
...
- '[List of MySQL data types](https://dev.mysql.com/doc/refman/8.0/en/data-types.html)'
---
...
MySQL uses the *Structured Query Language* (SQL) programming language. As a relational database, it stores and organizes data in tables. Tables structure the actual data inside tables as a series of rows, with each row consisting of one or more columns. Each row represents a different entry in the table, while each column contains one data field within the entry. The fields of data in these tables can be related to one another, and these relations help structure and organize the database. Specialized SQL statements permit clients to add, delete, modify, and retrieve data.
...
To install the MySQL server on Ubuntu, follow these steps. Ensure the system is up-to-date as explained in the "Before You Begin" section.
...
### Configuring the MySQL Server on Linux
...
### Logging in to MySQL as the Root User
...
MySQL uses standard SQL syntax for all of its commands. This section demonstrates how to perform basic tasks in MySQL, such as creating databases and tables and adding data. For full information on how to use MySQL, see the [*MySQL Reference Manual*](https://dev.mysql.com/doc/refman/8.0/en/). For a brief but thorough introduction, try the [*MySQL tutorial*](https://dev.mysql.com/doc/refman/8.0/en/tutorial.html).
...
5 rows in set (0.00 sec)
    ...
    {{< /output >}}
4.  To work with a database, tell mySQL to start using it. This is accomplished with the `USE` command.
...
The main way to insert a new row of data to a table is with the `INSERT` command.
...
2 rows in set (0.00 sec)
    {{< /output >}}
3.  It is also possible to only select rows fitting a particular criteria, for example, where a column is set to a certain value. Use the `WHERE` keyword as a qualifier, followed by the match criteria as a constraint. In this example, only rows in which `column2` is set to `b` are displayed.
