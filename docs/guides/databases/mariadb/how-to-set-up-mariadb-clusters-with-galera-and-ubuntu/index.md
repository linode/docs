...
Some of the main advantage of Galera Cluster are as follows:
...
1.  If all servers are located inside the same data center, consider using private IP addresses in the Galera Cluster configuration files. This enhances data security.
...
3.  Depending on the specific release of MariaDB being used, it might be necessary to install the `galera-4` package. This includes the Galera write-set replication provider library. On many systems, this package is already installed.

    ```command
    sudo apt-get install galera-4
    ```

4. MariaDB is activated upon installation. Use the `systemctl status` command to confirm MariaDB is `active`.
    ...
    Active: active (running) since Thu 2023-03-30 13:08:43 UTC; 3min 52s ago
    ```

5.  Use `mysql_secure_installation` to improve database security. The root password can be left unchanged. Unix socket authentication is also not required. However, you should answer `Y` to the following questions:
...
6.  Repeat the steps in this section for every node in the cluster.
...
* The `wsrep_cluster_address` setting must contain a list of all the IP addresses in the cluster. For a two-node set-up, it must be set to `gcomm://node1-ip-address,node2-ip-address,node3-ip-address`. If there are three nodes, it would be `gcomm://node1-ip-address,node2-ip-address,node3-ip-address`, and so forth.
...
2.  Add the following contents to the file, making the following adjustments.

    - For the `wsrep_cluster_address` setting, change `linode1-ip-address` to the IP address of the current Linode. Change `linode2-ip-address` and `linode3-ip-address` to the IP addresses of the other Linodes in the cluster, as applicable.
    - Change the value of `wsrep_node_address` to the IP address of the first Linode. This must be the same address as `linode1-ip-address`.
    ...
    The file should resemble the following example file.
    ...
    wsrep_cluster_address="gcomm://linode1-ip-address,linode2-ip-address, linode3-ip-address"

    # Galera Synchronization Configuration
    wsrep_sst_method=rsync

    # Node Configuration
    wsrep_node_address="linode1-ip-address"
...
4.  Edit the `galera.cnf` file on the next server. The contents should closely resemble the `galera.cnf` file on the first server. However, `wsrep_node_address` must contain the IP address of the second node. Choose a different node name for `wsrep_node_name`.
    ...
    # Node Configuration
    wsrep_node_address="linode2-ip-address"
...
5.  Execute a SQL query to retrieve `wsrep_cluster_size`. The value should have increased to `2`. This query can be run on either the first or second node.
...
6.  Activate the `mariadb` process on any remaining nodes in the cluster. After activation, run the `sudo mysql -u root -p -e "SHOW STATUS LIKE 'wsrep_cluster_size'"` query again. The value should increment each time a new cluster member is activated. After MariaDB has been restarted on all nodes, the cluster is fully operational.
...
7.  To confirm all databases are synched together, access the MariaDB application and run the following SQL query. The output should display `Synced`.
...
It is relatively easy to ensure the cluster is working correctly. Configure a database entity, such as a new database or table, on one node, and confirm it is replicated to the other nodes. To confirm the cluster is operational, follow these steps.
...
3.  Switch to the new database and create a simple table. The table should contain a primary key.
...
5.  On a different node in the cluster, access MariaDB and list the databases. The `test1` database should be included even though it was created on a different node.
...
6.  Switch to the `test1` database and display the description of the `States` table. If it is equivalent to the table on the first server, replication is working as expected.
...
7.  Repeat the previous two steps on each node in the cluster. Ensure the table definition is present on all nodes.

8.  A database, table, or row is replicated to all servers in the cluster no matter which node was used to create it. To further test the cluster, create a database or table on the second node and ensure it is still replicated properly to the other nodes.
...
2.  On the first node, allow TCP and UDP connections on four designated ports from the second node in the cluster. Use the format `sudo ufw allow from remote_addr to any port 3306,4567,4568,4444 proto tcp` and `sudo ufw allow from remote_addr to any port 3306,4567,4568,4444 proto udp`. Specify the IP address of the second node in place of `remote_addr`. For instance, the following commands allow MariaDB Galera Cluster access from `192.168.132.33`.
