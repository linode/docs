...
modified_by:
  name: Linode
title: 'Visualize Apache Web Server Logs Using the Elastic Stack on Debian 10'
...
3.  Follow the steps in our [Apache Web Server on Debian 10 (Jessie)](/docs/guides/how-to-install-apache-web-server-debian-10/) guide to set up and configure Apache on your server.
...
1.  Install the default version of Java available on Debian 10:
...
In order to collect Apache access logs, Logstash must be configured to watch any necessary files and then process them, eventually sending them to Elasticsearch. This configuration file assumes that a site has been set up according to the previously mentioned [Apache Web Server on Debian 10 (Jessie)](/docs/guides/how-to-install-apache-web-server-debian-10/) guide to find the correct log path.
...
4.  Next, open Kibana in your browser at [http://localhost:5601](http://localhost:5601). The landing page should look similar to the following. Click on "Explore on my own" to begin configuring Kibana.
    ...
    ![Kibana 7 Initial](elastic-stack-debian-10-kibana-initial.png "Kibana 7 Initial")
...
5.  From the next page, click on "Connect to your Elasticsearch index" under "Use Elasticsearch data" to configure an index pattern in Kibana.
    ...
    ![Kibana 7 Index Pattern Link](elastic-stack-debian-10-kibana-index-pattern-link.png "Kibana 7 Index Pattern Link")

6.  Close the information sidebar on the Index Pattern configuration page.

    ![Kibana 7 Index Pattern About](elastic-stack-debian-10-kibana-about-close.png "Kibana 7 Index Pattern About")

7.  At the upper right of the interface, select the "Create index pattern" button.

    ![Kibana 7 Create Index Pattern](elastic-stack-debian-10-kibana-create-index-pattern.png "Kibana 7 Create Index Pattern")

8.  On the "Create index pattern" page, enter the text `logstash*` into the "Index pattern name" field. Click "Next step" to continue.

    ![Kibana 7 Create Logstash Index](elastic-stack-debian-10-kibana-logstash-index.png "Kibana 7 Create Logstash Index")
...
9.  From the "Time field" drop down menu, select `@timestamp` as the time field. This corresponds with the parsed time from web server logs. Click the "Create index pattern" button to continue.
    ...
    ![Kibana 7 Select Time Field](elastic-stack-debian-10-kibana-select-time-field.png "Kibana 7 Select Time Field")
...
![Kibana 7 Open Sidebar](elastic-stack-debian-10-kibana-sidebar.png "Kibana 7 Open Sidebar")

Select the "Discover" menu item.

![Kibana 7 Discover Tab](elastic-stack-debian-10-kibana-discover.png "Kibana 7 Discover Tab")

The Discover interface should show a timeline of log events:

![Kibana 7 Discover View](elastic-stack-debian-10-kibana-discover-view.png "Kibana 7 Discover View")
...
![Kibana 7 Document Fields](elastic-stack-debian-10-kibana-field-dropdown.png "Kibana 7 Document Fields")
...
![Kibana 7 Search Bar](elastic-stack-debian-10-kibana-discover-search.png "Kibana 7 Search Bar")
...
![Kibana 7 Visualize Tab](elastic-stack-debian-10-kibana-visualize-tab.png "Kibana 7 Visualize Tab")

Then, select the "Create new visualization" button:

![Kibana 7 Create Visualization](elastic-stack-debian-10-kibana-create-visualization.png "Kibana 7 Create Visualization")

Select "Pie" to create a new pie chart. Note that you may need to scroll down to find the "Pie" visualization:

![Kibana 7 Select Pie Chart Visualization](elastic-stack-debian-10-kibana-pie-chart.png "Kibana 7 Select Pie Chart Visualization")

Then select the `logstash-*` index pattern to determine from where the data for the pie chart will come:

![Kibana 7 Select Pie Chart Index](elastic-stack-debian-10-kibana-vis-index.png "Kibana 7 Select Pie Chart Index")
...
![Kibana 7 Select Pie Chart Configuration](elastic-stack-debian-10-kibana-finished-pie.png "Kibana 7 Select Pie Chart Configuration")
