---
slug: using-redis-client-side-caching
author:
...
Redis is an open-source in-memory database with a reputation for working exceptionally well for caching data on web applications. Redis's fast transactions and low latency help to increase application performance while its server-assisted caching feature make client-side caching more efficient.
...
To elaborate, an application using Redis for server-assisted caching should have layering like this:
...
Further on, you can find some additional options for Redis's client tracking. This includes a broadcast mode, for tracking sets of keys, and some parameters for more finely controlled cache tracking.
...
Each option has its use cases, although the later is only available on Redis versions from 6 onward.

Below, you can see the steps used for setting up your clients with each of these options.

#### Dedicated Client for Notifications

1. Create a Redis client by making an authenticated connection to the Redis server. See our guide on **Connecting to Redis and Using Redis Databases** for instructions on doing so.

1. Determine the client's ID using the following command. This ID gets used in setting up client tracking in one of the subsequent steps, so keep note of it. This and subsequent examples use `15` for the notification client's ID:
...
The second client, and any subsequent clients set up likewise, is now being tracked for server-assisted caching. The server keeps note of any data that these tracked clients fetch. Then, the server sends invalidation messages to the listening client (the first one above) whenever any of that data changes.

You can verify this by issuing the following commands on the second client:
...
Redis 6 introduced the RESP3 protocol with the aim of providing more capabilities for Redis clients.

Among these is the ability to receive messages on an active client. Previously (under the RESP2 protocol), a Redis client could only receive messages by subscribing to a channel. This left the client in listening mode, unable to issue commands.
...
For versions prior to version 6, see the previous section for creating a dedicated client for invalidation messages.
{{< /note >}}

1. Create a Redis client by making an authenticated connection to the Redis server. See our guide on **Connecting to Redis and Using Redis Databases** for instructions on doing so.

1. Switch the client to RESP3:
...
1. Turn on client tracking for the client:

        CLIENT TRACKING on

The client now has tracking enabled. Any data the client fetches gets monitored by the server. When that data changes, the client receives a notification to invalidate its cached data.

You can verify this by fetching a piece of data and making a change to it after:
...
Broadcast mode also supports multiple prefixes. For instance, modifying the example above:

    CLIENT TRACKING on BCAST PREFIX cat_one: PREFIX cat_two: PREFIX cat_three: REDIRECT 15

### Additional Settings

Redis's client tracking comes with some additional parameters that let you fine tune how the server tracks clients and sends notifications. The next sections explain what these parameters are and how you can use them.
...
- Using `OPTIN`, Redis only tracks keys fetched immediately after a `CLIENT CACHING yes` command. In this example, the `cat_one:key_one` key is not tracked but the `cat_one:key_two` key is:
...
- Using `OPTOUT`, Redis tracks as usual but allows you to use the `CLIENT CACHING no` command to exclude any key fetched immediately after from being tracked. This next example tracks `cat_one:key_one` but not `cat_one:key_two`:
...
Some examples in the sections above showed you invalidation notifications for changes made by the tracked client itself. You can disable this behavior using the `NOLOOP` option for the `CLIENT TRACKING` command.

With this option, invalidation notifications are only provided when a different client makes a change to a tracked key.

For example, here, Redis would not provide an invalidation notice to the client:
