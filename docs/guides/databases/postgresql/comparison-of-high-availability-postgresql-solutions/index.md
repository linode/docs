...
og_description: 'This guide describes the high availability and resiliency options for PostgreSQL, with a survey of the three most common replication managers.'
...
PostgreSQL is a powerful, flexible, and reliable relational database system. But it does not automatically implement high availability. This means it remains vulnerable to network outages or server failures. Users must take additional steps to enable resiliency and ensure their databases are consistently operational.

This task is typically accomplished through the use of specialized replication manager applications. A replication manager works in conjunction with the existing PostgreSQL infrastructure. It automatically handles system monitoring and initiates switchover events when necessary. If the primary copy of the database goes offline, it redirects users to a backup copy. Some applications can even take steps to restore a failed system or database process.
...
{{< note >}}
In PostgreSQL terms, high availability is an attribute of a single cluster of databases, so the entire system is a *high availability cluster*. To avoid confusion, this guide uses the term high availability/HA cluster to refer to the collection of distinct database instances containing the same data.
{{< /note >}}
...
For a concise discussion and comparison of the three main alternatives, see the Akamai blog about PostgreSQL high availability.
...
[Patroni](https://patroni.readthedocs.io/en/latest/) is a Python-based software template for enabling high availability on PostgreSQL databases. This framework requires some template customization to work most effectively. It also requires a *distributed configuration store* (DCS), but supports a number of different storage solutions. Patroni works well on a two-node HA cluster consisting of a primary node and a single replica.

Patroni configures a set of nodes into a HA cluster and configures streaming replication to share updates. It runs an agent on each node in the HA cluster to share node health updates between the members. The primary node is responsible for regularly updating the *leader key*, which is stored in the DCS. If it fails to do so, it is evicted as the primary and another node is elected to take over. After a switchover, the replicas coordinate their position in respect to the database updates. The most up-to-date node typically takes over. In the event of a tie, the first node to create a new leader key wins. Only one node can hold the leader key at any time. This reduces any ambiguity about the identity of the primary node and avoids a split brain scenario.
