...
This guide provides an introduction to [*PostgreSQL*](https://www.postgresql.org/), an open source *object-relational database management system* (RDBMS). PostgreSQL builds upon the original *Structured Query Language* (SQL) specification with many new features, emphasizing compliance. PostgreSQL transactions are ACID-compliant, meaning they are atomic, consistent, isolated, and durable. PostgreSQL is one of the most popular database systems, and is available for most operating systems, including Ubuntu 20.04.
...
The easiest way to install PostgreSQL on Ubuntu is with the package installation program `apt`. This method installs the latest version of PostgreSQL that is included in the Ubuntu packages (currently version 12.5).
...
4.  If you want PostgreSQL to automatically launch upon system boot-up, register it with `systemctl`.
...
5.  Install the latest version of PostgreSQL.
    ...
    {{< note >}}
To install an earlier release of PostgreSQL, add the release number as a suffix, for example `install postgresql-11`.
    {{< /note >}}
6.  Ensure PostgreSQL is running with `systemctl`.

        sudo systemctl start postgresql.service
7.  If you want PostgreSQL to automatically launch upon system boot-up, register it with `systemctl`.
...
testdatabase=#
    {{< /output >}}
3.  You can list all of the databases in the cluster with the `\l` meta-command.
...
--------------+----------+----------+-------------+-------------+-----------------------
 ...
 testdatabase | postgres | UTF8     | en_US.UTF-8 | en_US.UTF-8 |
    {{< /output >}}
4.  Whenever you are logged into PostgreSQL, you can always find out what database you are logged into, along with the name of your role, with the `conninfo` meta-command.

        \conninfo
    The command returns basic information about your database.
{{< output >}}
You are connected to database "postgres" as user "postgres" via socket in "/var/run/postgresql" at port "5432".
{{< /output >}}
5.  While you are logged into PostgreSQL, you can switch to a different database with the `\c` meta-command.

        \c testdatabase
6.  If you are absolutely certain you do not need a database any longer, you can delete it with the `dropdb` command. Issue this command from the Linux shell while logged in as `postgres`.
...
3.  You can list all of the tables in the database with the `\dt` meta-command.
...
4.  You can show the complete schema of the `customers` table, including all of the columns and data types, with the `\d+` meta-command.
...
3.  You can view the contents of a table with the `SELECT` command.
...
4.  To return a subset of the table columns, list the specific columns you want to retrieve in place of the `*` symbol.
...
We strongly recommend you finalize your complete database schema before implementing it. Nevertheless, there might be times when you have to change your table structure after the fact. PostgreSQL provides the ability to add or delete columns from existing tables, even if they are already populated with data.
...
For a small single-user database, you might only require the default PostgreSQL account. However, for most applications, you should create a separate account for each user. You can create new PostgreSQL users with the following process.
...
5.  A user's role can be expanded with the `ALTER ROLE` command. Issue the following command as the     `postgres` user to allow `testuser` to create databases.

        ALTER ROLE testuser CREATEDB;
    The `\du` command now shows the new privilege.
    {{< output >}}
List of roles
Role name |                         Attributes                         | Member of
-----------+------------------------------------------------------------+-----------
...
We do not recommend opening up PostgreSQL to listen for connections on public IP addresses because this poses a security risk. If you want to access PostgreSQL remotely, you can use a graphical user interface called pgAdmin. See one of the Linode guides to pgAdmin for more information.
