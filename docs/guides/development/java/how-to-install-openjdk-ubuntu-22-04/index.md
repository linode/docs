...
h1_title: "How to Install OpenJDK on Ubuntu 22.04"
enable_h1: true
contributor:
  name: Jeff Novotny
external_resources:
...
OpenJDK is an alternative to the official Oracle Java implementation, which has a more restrictive license. OpenJDK is offered under the *GNU General Public License* (GPL) 2.0 license. With the exception of the licensing details, OpenJDK and Java SE are almost exactly the same.
...
3.  Install the preferred release of OpenJDK using `apt`. Answer `y` when Ubuntu asks whether to proceed with the installation. The following command demonstrates how to install the full JDK platform of OpenJDK release 17. To install only the JRE component, see the following instruction.
...
4.  **(Optional)** To install only the JRE component of OpenJDK, use the following command. To install a release other than OpenJDK release 17, change `17` to the appropriate release number.
...
6.  If the full JDK platform is installed, confirm the release of the Java compiler.
...
Although Java is already installed, further configuration is required. Setting a few environment variables makes OpenJDK easier to use and allows other applications to find it. To set the Java environment variables, follow these steps.
...
3.  Echo `JAVA_HOME` and `PATH` to verify they are set correctly. The `$JAVA_HOME` value should be similar to `/usr/lib/jvm/java-17-openjdk-amd64`, depending on the release. This value should also be appended to the end of the `$PATH` variable.

        echo $JAVA_HOME
        echo $PATH

## Testing the OpenJDK Installation
...
### Running a Java Application Using the OpenJDK JRE
...
2.  In the same directory, use the OpenJDK JRE to run `HelloWorld.java`.

        java HelloWorld.java

3.  Verify the console output to ensure the installation was successful. The `Hello World from Java!` text should appear. Because JRE must first compile the program into bytecode, there might be a short delay before the program starts running.

    {{< output >}}
Hello World from Java!
    {{< /output >}}

### Compiling a Java Application Using the JDK

The JDK is a full development environment. It can compile Java code into Java bytecode for faster execution. This process creates a stand-alone Java class file that can be shared with others. To compile the `HelloWorld.java` test application, follow these steps.

1.  From the directory containing the source code, use the `javac` compiler to compile the file.

        javac HelloWorld.java

2.  Confirm a `HelloWorld.class` file now exists in the same directory.
...
3.  Run the compiled `HelloWorld` class file using the OpenJDK JRE. The output `Hello World from Java!` should appear.
