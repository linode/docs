...
A remote procedural call is a central concept in distributed computing. It allows one system to call a function in a different address space, or even on a completely different system. If the client and server are running on the same system, they use distinct virtual addressing spaces. Client systems only require a description of the shared interface to interact with the remote system.
...
1.  The client calls a local stub function which is responsible for communicating with the server.
2.  The stub function invokes a system call to transmit the message, including the necessary function parameters as arguments.
3.  The client sends the message to the server, which is either on the same system or on a remote component.
4.  The server receives the message and passes it to a server stub.
5.  The server stub processes the message and extracts the parameters.
6.  The server stub calls a function to handle the request and the task runs to completion.
7.  When processing is complete, the reply is transmitted. The previous six steps reoccur in the opposite direction. The server calls a stub function to transmit the message. The client receives the message and finally passes the information to the stub that originally made the request.

## What is gRPC?

gRPC is a specific implementation of RPC. It allows different machines, running applications that might be written in different languages, to communicate. The simplicity and efficiency of its design is useful for implementing microservices, where the server provides specific and finely-tuned services to multiple independent clients.

gRPC accomplishes this task through the use of protocol buffers, also known as `protobufs`. Protocol buffers were originally developed by Google. They are small and efficient, and can be transmitted and processed quickly. These buffers provide gRPC with an *interface Definition Language* (IDL) and a message-exchange format. More information about protocol buffers can be found on the [*Google Developers Page for Protocol Buffers*](https://developers.google.com/protocol-buffers/docs/overview). The Google site also include a [*Protocol Buffers Language Specification*](https://developers.google.com/protocol-buffers/docs/reference/proto3-spec).
...
*   **Bidirectionally-streaming RPC:** This architecture combines aspects of the response-streaming and request-streaming RPCs. Both the client and server transmit a stream of messages in an asynchronous and independent manner, and process the messages independently.

## Advantages of gRPC

gRPC has many advantages over traditional client-server architectures.

*   It permits the development of highly-scaled and distributed systems in an incremental manner.
...
*   It yields good performance and low latency, partly due its use of HTTP/2. The protocol buffers are efficient and lightweight, and can be transmitted and processed quickly.
...
## Creating a gRPC Project in Python for Remote Procedural Calls
...
1.  **Installing Python:** If Python is already installed on the system, skip to the second step.
2.  **Installing the Python gRPC Tools:** These tools install the `protoc` compiler and other necessary components.
3.  **Creating the Proto File and Generating the Python Stubs:** All RPC projects start with a `.proto` file containing definitions for the service interface, including the message types and methods. The `protoc` compiler is used to generate two infrastructure files for the client and server.
4.  **Implementing the gRPC Server:** The gRPC server code launches the server and implements the methods from the service interface. A function is required for each method specified in the interface.
5.  **Implementing the gRPC Client:** The client code includes Python stub functions to initiate a connection to the server, generate requests, and receive the responses.
6.  **Running and Testing the gRPC Application:** Launch the server, and start the client. Ensure the application generates the expected results.
...
1.  Verify whether Python is installed on the Linode using the following command. If version 3.7 or later of Python is installed, skip to the next section.

        python3 ––version
2.  Refresh the Ubuntu repository.

        sudo apt update
3.  Install the Python application along with its `pip` component.

        sudo apt install python3-dev python3-pip
4.  Confirm the version of both Python and `pip`. Version 9.0.1 or higher of `pip` is required to run gRPC.
...
### Installing the Python gRPC Tools
...
Successfully installed grpcio-1.38.0
    {{< /output >}}
2.  Install the gRPC tools. This also installs the protocol buffer compiler `protoc`.
...
### Creating the Proto File and Generating the Python Stubs
...
1.  Create a main directory for the project named `teams`. Inside this directory, create another directory named `protobufs`.
2.  Change the working directory to `teams/protobufs` and create a file named `teams.proto`.

        cd teams/protobufs
        touch teams.proto
3.  The `teams.proto` file contains a shared API that the client and server can use to communicate with one another. At the top of the file, declare the `syntax` attribute as `proto3`.
    {{< note >}}
...
message TeamResponse {
  ...
  string nickname = 2;
   ...
   {{< /file >}}
5.  The final section of the file defines the  `Teams` service and any shared `rpc` methods. In this case, `Teams` only contains the `GetTeam` method. This method accepts a message parameter of type `TeamRequest` and returns a message of type `TeamResponse`. The client is able to call this function on the server using an auto-generated stub method.
...
service Teams {
  ...
  rpc GetTeam (TeamRequest) returns (TeamResponse) {}
   ...
   {{< /file >}}
6.  Here is an overview of the entire `teams.proto` file.
...
2.  At the top of the file, `import` the required packages. To import the auto-generated classes, add an `import` statement corresponding to the filename of each auto-generated file, minus the `.py` extension.
...
import teams_pb2_grpc
    {{< /file >}}
3.  Create the main class, following these guidelines:
    ...
    * The function itself is simple and straightforward. The information from the client is referenced through the `request.city` field. The `city` field was defined within the `TeamRequest` message declaration inside `teams.proto`.
...
class Teams(teams_pb2_grpc.TeamsServicer):
    ...
    {{< /file >}}
4.  The server must handle all client calls. This is the responsibility of the `serve` function. It creates the server, attaches a `servicer` to it, and tells it to listen for incoming messages on port `50051`. The `serve` function follows the same basic format in every gRPC application. The second line of the `serve` function is somewhat complicated. It uses the `add_TeamsServicer_to_server` method from the `teams_pb2_grpc` class. This method takes a `Teams` object and the `server` itself as parameters.
    {{< file "~/teams/teams_server.py" python >}}
def serve():
    ...
    {{< /file >}}
5.  The file ends with a simple `main` function that only exists to call `serve`. Although `server.start` is non-blocking and runs inside its own thread, `wait_for_termination` blocks the caller as long as the server is running.
    {{< file "~/teams/teams_server.py" python >}}
if __name__ == '__main__':
    logging.basicConfig()
    serve()
    {{< /file >}}
6.  The entire `teams_server.py` file is shown here for reference.
...
2.  At the top of the file, `import` the required packages. This section is similar to the corresponding section of the server file.
...
import teams_pb2_grpc
    {{< /file >}}
3.  Create the `run` function. This function is not part of any class, although it uses classes from the auto-generated files.
    ...
    *   Create a local stub object using `teams_pb2_grpc.TeamStub`, passing it the channel. This stub is used to call the methods defined in the `.proto` file.
...
def run():
    with grpc.insecure_channel('localhost:50051') as channel:
        ...
        print("Teams client received: " + response.city + response.nickname)
    {{< /file >}}
4.  The main function logs some information and then calls `run`. The `run` function is non-blocking, so when it is finished, the program ends.
    {{< file "~/teams/teams_client.py" python >}}
if __name__ == '__main__':
    logging.basicConfig()
    run()
    {{< /file >}}
5.  The entire `teams_client.py` file is shown here.
...
### Running and Testing the gRPC Application
...
2.  Open a new console on the Linode. Change to the `teams` directory, and run the `teams_client.py` program.

        cd teams
        python3 teams_client.py
3.  The `teams_client.py` application displays the results of the remote procedural calls it made to the server. The program terminates when it has printed all of the results.
...
Teams client received: Miami not a member
    {{< /output >}}
4.  Return to the window where you launched the `teams_server` application. The program should still be running. The contents of the window should display the metadata from the client requests.
...
{'user-agent': 'grpc-python/1.38.0 grpc-c/16.0.0 (linux; chttp2)'}
    {{< /output >}}
5.  When you are finished using the gRPC client, enter `Ctrl-C` in the same console to terminate the application. The server can be restarted at any time.