...
description: 'This guide shows you how to get started creating animations using the Angular web framework'
...
1. Follow the steps in our [Getting Started with Angular](/docs/guides/angular-tutorial-for-beginners/#getting-started-with-angular) guide to installing Node.js, the Node Version Manager (nvm), and Angular. As a result of following these steps, you should have a directory named `example-app` in your home folder.
...
In your preferred text editor, open your Angular application's `src/app/app.component.ts` file and add the `animations` metadata property to the declared component. The majority of the code used to animate the `<div>` element is contained in the `animations` metadata option.
...
Your Angular project already contains a component HTML template file. The `app.component.ts` component file points the location of the `./app.component.html` file in the `templateURL` metadata. In this section, you add the HTML to animate with Angular.
...
Angular animation *states* specify the change that you want apply during your animation. For the example, the `<div>` background color's state should change from one color to another. You use Angular's `state()` function to name the state and define the element's CSS styles that are associated with the state.
