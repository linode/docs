...
Python-Markdown is a free open source Python library for converting Markdown code to HTML markup. It can either accept interactive input or process a Markdown file. As much as possible, Python-Markdown follows the rules of the original Markdown specification. Its behavior makes it suitable for use in web server environments.
...
- It does not raise any exceptions or write any errors to standard output.
...
The Python-Markdown library can be installed using `pip`, a Python package for installing libraries. To install Python-Markdown, follow these steps. These instructions are designed for Ubuntu 22.04, but are generally applicable to all Linux distributions.
...
The second program takes an even simpler approach. It invokes the `markdownFromFile` method to convert the Markdown file to HTML in a single step.

1.  To use the first method, create a file containing the original Markdown text. `List.md` file is a Markdown file that expands the "To Do" list used in the previous section.
...
4.  Print the contents of `List.html` to ensure the Markdown code was translated correctly.
...
{{< file "mdAutoConverter.py" python >}}
import markdown
