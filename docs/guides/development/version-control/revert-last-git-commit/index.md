...
Git is a widely used version control system (VCS) known for its versatility. Git utilizes local clones of central repositories to bring more effective collaboration. And it keeps track of all the committed changes along the way so that things are readily recoverable.

One instance where you might need that recoverability is after an inadvertent commit or when, for other reasons, you need to undo the most recent commit. Git has the ability to revert the last commit, and this tutorial shows you exactly how. It covers methods using the `revert` command and the `reset` command and explains the differences.
...
And for a more general, and thorough, coverage of reverting Git commits, take a look at our guide on [How to Undo a Git Commit](/docs/guides/how-to-undo-git-commit/).
...
The steps presume you have already installed Git and done basic configuration (e.g., user email address and name). If you have not done those things, you can learn how in our guide [How to Install Git and Clone a GitHub Repository](/docs/guides/how-to-install-git-and-clone-a-github-repository/).
...
1. Create a new directory for your Git repository, and change into that directory. Here, the new directory, `git-example` is created in the current user's home directory:
    ...
    From here through the rest of the guide, you should execute the given commands while working in this directory.
...
Git starts a new commit to revert the changes, and it may present you with a text editor allowing you to edit the description for the new commit. When you are content with the description, exit the text editor to complete the reversion.
...
- Use the `--hard` option to likewise roll back to a previous commit. However,this option results in all file changes being reverted as well. Changes you have made to files in the working directory and staged changes are discarded.

        git reset --hard HEAD~1

- Use the `--mixed` option to, again, roll back to a previous commit. Here, a middle ground is adopted for files changes. As with the `--hard` option, changes made to files in the working directory are discarded. However, like the `--soft` option, staged changes are preserved.
...
That covers all you need to revert recent Git commits. Moreover, the techniques covered in this tutorial can also help you more generally to manage Git commits. The `revert` command can be useful for precisely removing past commits while retaining your commit history. The `reset` command, on the other hand, provides a more radical option, completely reverting a repository to a previous commit, including the commit history.
