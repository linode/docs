...
description: "WebAssembly lets you develop web applications that run at near-native speeds. WebAssembly even allows you to compile web applications from languages like C/C++ and Rust. This guide explores what WebAssembly is, how it works, and how you can get started with it."
og_description: "WebAssembly lets you develop web applications that run at near-native speeds. WebAssembly even allows you to compile web applications from languages like C/C++ and Rust. This guide explores what WebAssembly is, how it works, and how you can get started with it."
...
1. Update your system. On Debian and Ubuntu distributions, you can do this with:
...
The potential WebAssembly offers is still being explored, and the ecosystems to support it are still being built out. But already, WebAssembly promises to be a strong option for developing client web applications.
...
### Creating and Compiling a WebAssembly Web Project
...
1. Use `cargo-generate` to create a base Rust WebAssembly project. Be sure to first navigate to the directory where you would like to store your project's directory. In this case, that is the current user's home directory:
    ...
    Enter a name for your project when prompted. This guide uses the name `hello-wasm-world`, so replace that name in the following steps with the name you select.
...
### Running the WebAssembly Web Application
...
1. Open up the `package.json` file (the one in the `www` subdirectory), and add a `dependencies` section with an entry for your WebAssembly `pkg` directory:
