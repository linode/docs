...
```command {title="Query for CIDR block associated with VPC"}
...
```command {title="Query for subnets belonging to a VPC"}
...
```command {title="Query for EC2 instances belonging to a VPC"}
...
```command {title="Query for route tables belonging to a VPC"}
...
```command {title="Edit /etc/netplan/01-netcfg.yaml on Bastion to set eth1 VLAN address"}
network:
  version: 2
  ethernets:
    eth0:
...
-   AWS
  ...
  -   [VPC Service](https://aws.amazon.com/vpc/)
