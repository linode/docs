...
1. On the **Rules** page, select the default behavior for both inbound and outbound traffic using the **Inbound Policy** and **Outbound Policy** dropdown menus. **Accept** will allow all traffic except for those defined in the listed rules, and **Deny** will block or deny all traffic except for those defined in the rules.
...
1. Provide the following Rule configurations:
    ...
    | **Action** | &bull; Choose whether this rule will be to allow or deny traffic. The action defined in specific rules will take precedence over the default inbound and outbound traffic policy. *Required* |
