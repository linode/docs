...
DNSSEC introduces a few new terms and record types. The resource record set (RRset) describes all resource records of a given type within a zone. For example, all A records within the zone linoderocks.com comprise a single RRset.
...
This guide uses the linoderocks.com domain as an example. Replace this address with your own in the examples that follow. It is authoritative for the zone linoderocks.com  (see “An Introduction to DNS on Linux” and substitute your domain name as appropriate).


Remove any previously installed keys and certificates in /etc/nsd, then generate new ones:
...
Restart NSD to ensure the server loads the new keys and certificates:


sudo systemctl restart nsd


Install the ldnsutils suite, needed for key generation and signing:

sudo apt install ldnsutils


Move to the zones directory and generate ZSK and KSK files. Switch to the root user for export commands because sudo won’t work here. You can optionally capture the ZSK and KSK variables for later reuse in the “Zone Maintenance” section.


cd /etc/nsd/zones/master
sudo su - root
export ZSK=`/usr/bin/ldns-keygen -a ECDSAP256SHA256 -b 1024 linoderocks.com`
export KSK=`/usr/bin/ldns-keygen -k -a ECDSAP256SHA256 -b 2048 linoderocks.com`
# optional capture of $ZSK and $KSK variables for later maintenance
echo $ZSK > linoderocks.com.zsk
echo $KSK > linoderocks.com.ksk
...
The KSK command above generated a delegation signing (DS) record, but you will soon create a separate DS record, and can delete this one:


rm *ds


While still logged in as root, sign the linoderocks.com zone using the ZSK and KSK variables you previously created.

ldns-signzone -n -s $(head -n 1000 /dev/urandom | sha256sum | cut -b 1-16) linoderocks.com.zone $ZSK $KSK


In addition to keys, the zones directory now contains a signed zone file.
...
Open the main /etc/nsd/nsd.conf configuration file and, in the zone: section, point to the signed zone file.

zone:
..
zonefile: "zones/master/linoderocks.com.zone.signed"
..


Save and close the file. Then reload the NSD configuration and signed zone file.

nsd-control reconfig
nsd-control reload linoderocks.com
# exit as root user
exit

A dig query for the zone using DNSKEY now return records with DNSSEC signatures::

dig DNSKEY @ns1.linoderocks.com linoderocks.com +multiline +norec
..
linoderocks.com.	3600 IN	DNSKEY 256 3 13 (
...
linoderocks.com.	3600 IN	DNSKEY 257 3 13 (
...
Generate DS records for the zone, and save the results to a text file or to your clipboard. You need these for the final step of having your zone signed at your domain registrar.

cd /etc/nsd/zones/master
sudo ldns-key2ds -n -f -2 linoderocks.com.zone.signed
linoderocks.com.	3600	IN	DS	6274 13 2 044783c65c032a0ae25a1de626e341c483a89601c766e812a001bc512145fc81
linoderocks.com.	3600	IN	DS	55738 13 2 c4dae4d001f8c8f1b4f1adec890eba39010143752e6ce03b6567c85aa7fbde46


Your server does not return valid responses until you upload these DS records at your registrar. For each DS record, add this information at the registrar:
...
Next, verify your configuration with a DNSSEC test site such as dnsviz.net.
...
For the $ZSK and $KSK variables in this command, key in the names of your current ZSK and KSK files, without the filename extensions. Then reload the zone:
