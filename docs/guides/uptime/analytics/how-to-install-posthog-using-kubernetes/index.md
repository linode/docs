---
slug: how-to-install-posthog-using-kubernetes
author:
  name: Linode Community
  email: docs@linode.com
description: 'Learn how to self-host PostHog, an all-in-one analytics tool, on LKE to measure and track useful usage and funnel statistics and insights.'
og_description: 'Learn how to self-host PostHog, an all-in-one analytics tool, on LKE to measure and track useful usage and funnel statistics and insights.'
keywords: ['PostHog','Product analytics','Funnel analysis','Install PostHog LKE','Install PostHog Linode']
license: '[CC BY-ND 4.0](https://creativecommons.org/licenses/by-nd/4.0)'
published: 2021-11-02
modified_by:
  name: Linode
title: "How to track funnel and product analytics with PostHog on Linode Kubernetes Engine | Linode"
h1_title: "How to track funnel and product analytics with PostHog on Linode Kubernetes Engine | Linode"
enable_h1: true
contributor:
  name: Jeff Novotny
external_resources:
...
- '[PostHog integration instructions](https://posthog.com/docs/integrate)'
- '[PostHog tutorials](https://posthog.com/docs/tutorials)'
- '[Helm v3 site](https://helm.sh/)'
- '[Wikipedia page on Funnel Analysis](https://en.wikipedia.org/wiki/Funnel_analysis)'
---

[*PostHog*](https://posthog.com/) is an open source product analytics tool that can replace Google Analytics. PostHog can be self-hosted on the [Linode Kubernetes Engine](https://www.linode.com/docs/products/compute/kubernetes/) (LKE) and installed using `cubectl` and [*Helm 3*](https://helm.sh/). PostHog offers a wide variety of features including funnel analysis, product use trends, and session recordings. This guide introduces PostHog and explains how to install and configure it on the Linode LKE.
...
Here are some of Posthog's most popular analytics features. A full list of features can be seen in the [*PostHog User Guide*](https://posthog.com/docs/user-guides).
...
*   PostHog supports funnel analysis. Funnel analysis is a method for understanding the steps required to reach a desired outcome and the number of users who reach each step. PostHog allows users to create a bucket for users who drop out at any point in the funnel.
...
## Installing PostHog on the Linode Kubernetes Engine
...
To install PostHog on a Linode LKE cluster, the system must satisfy the following requirements:

*   A Kubernetes cluster must have been created using Linode's LKE. To create a cluster, see the [Linode Kubernetes Engine Documentation](https://www.linode.com/docs/products/compute/kubernetes/). PostHog requires Kubernetes release 1.19 or newer.
*   The cluster must have enough resources to run PostHog. A minimum of 4 virtual CPUs and 8GB of memory is recommended.
...
*   To install PostHog, first install [*Helm v3*](https://helm.sh/). Helm is a client that acts as a package manager for Kubernetes. It is used to download, install, and upgrade applications on a cluster. Applications are installed using a Helm chart, which describes the essential resources using a common format. See the [Linode guide to Helm](https://www.linode.com/docs/guides/how-to-install-apps-on-kubernetes-with-helm-3/) for more information about Helm.
...
2.  Change the permissions on the script.

        chmod 700 get_helm.sh
3.  Run the installation script. To install Helm into `/usr/local/bin`, provide the `sudo` password.
...
helm installed into /usr/local/bin/helm
    {{< /output >}}
4.  View the release information to confirm Helm is installed correctly.
...
### Creating the values.yaml File and Installing PostHog
...
*   **ingress/nginx/enabled:** Set this value to `true` to enable the web server and load balancer.
...
2.  Add the following information to the file and save it. Replace `example.com` with the actual domain name.
...
3.  Add the PostHog repository to Helm.
...
5.  Use the `helm install` command to install and enable PostHog on the cluster.
...
1. Your application will be hosted at https://example.com
    {{< /output >}}
6.  It takes Helm and Kubernetes a few minutes to configure all of the PostHog components. The application is not ready until all components are configured.
7.  Verify the Kubernetes pods are all active. When all pods display a status of `running`, the installation is ready and complete.
...
posthog-worker-67b8f67b89-xfz8s                     1/1     Running   0          51s
    {{< /output >}}
8.  As an additional check, use the `helm status` command to verify PostHog is `deployed`.
...
### Obtaining the IP Address of the Node Balancer

The IP address of the node balancer is required to set the DNS records correctly. This is the address PostHog uses to send and receive network traffic. To determine the address of the node balancer, use the following command. The address is shown in the `EXTERNAL-IP` field.
...
2.  Edit the IP address of the domain record. Click the three dots to the right of the record and select **Edit**.
    ![Selecting a record to edit](select-dns-record-editing.png)

3.  Change the IP address in the `IP Address` field to the new address.

    ![Editing the DNS record](edit-dns-record.png)
...
{{< note >}}
The IP address of the node balancer cannot be used to access PostHog. The cluster is configured to restrict access to PostHog to the domain name.
{{< /note >}}

## Accessing PostHog

To access the PostHog website, enter the domain name in the browser. PostHog redirects any requests to the `https://yourdomainname.com/preflight` URL. This address displays the PostHog "preflight" page.
...
For information on how to configure and use PostHog, see the [*PostHog User Guide*](https://posthog.com/docs/user-guides) and [*integration overview*](https://posthog.com/docs/integrate).

### Troubleshooting PostHog Access Issues

If you cannot access the PostHog "preflight" page, there are certain steps to take to debug the installation. Here are a few concerns to investigate.
...
*   Ensure the information in the `values.yaml` file is correct.
*   Ensure the correct IP address is configured for the domain DNS record.
...
To disable and re-enable HTTPS, follow these steps:
...
kafka:
  persistence:
    size: 20Gi
    {{< /file >}}
2.  Upgrade the PostHog installation using the `helm upgrade` command.
...
1. Your application will be hosted at https://example.com
    {{< /output >}}
3.  Attempt to access PostHog again using the domain name. If the HTTPS configuration was the problem, the PostHog "preflight" page should now appear. The page displays a warning that PostHog is not correctly secured.
4.  Re-enable HTTPS to restore the encryption settings. Copy the backup file back to `values.yaml`.

        cp backup_values.yaml values.yaml
5.  Upgrade PostHog again to reconfigure HTTPS.

        helm upgrade -f values.yaml --timeout 20m --create-namespace --namespace posthog posthog posthog/posthog
6.  Try accessing the PostHog landing page again using the domain name. The page should now appear without any warnings.

## A Summary of PostHog

PostHog is an open source product analytics tool that allows users to self-host and control their software and their customer data. It is considered a solid alternative to Google Analytics, which has many restrictions and limitations. It is simple to install PostHog on a Linode LKE cluster using the Helm client.
