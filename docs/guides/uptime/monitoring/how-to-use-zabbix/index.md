...
og_description: 'This guide introduces the system monitoring application Zabbix, and explains how to perform some basic tasks.'
...
-   It includes an customizable and intuitive GUI for data visualization. Zabbix supports real-time graphs.
...
1.  Verify the status of the agent and ensure it is `active`.
...
After installation, add configuration to the `zabbix_agentd.conf` file. This allows the server to recognize the client and to transmit data using shared key encryption.
...
1.  **Optional** Display the key and copy it to someplace secure. The server requires this key to establish a connection with the server.
...
1.  Make the following changes.
    ...
    -   Change `Hostname` to the host name of the Zabbix **server**. This is the server name specified at creation time in the Linode Marketplace. In this example, the host name is `zabbixserver`. Do **not** use the hostname of the client here.
    ...
    -   Uncomment `TLSPSKFile` and set it to `/etc/zabbix/zabbix.psk`. This is the name of the file containing the key.

1.  The `zabbix_agentd.conf` should resemble this example. Change `server_IP` to the actual IP address of the Zabbix server. Leave the remainder of the lines unchanged.
...
1.  Zabbix displays the `host` configuration page. Under the **Host** tab, add the following information. Do not save the information yet.
    ...
    -   Enter the `Hostname`. This is the host name of the server running the agent, in this case, `zabbixtarget`.
    -   For the `Template` field, use `Linux by Zabbix agent`. Start typing this phase to narrow down the choices using auto-search.
...
1.  Select the **Encryption** tab from the top line of the configuration widget. Enter the following information.
    ...
    -   Enter the PSK key for the target in the `PSK` key. This is the key stored in `/etc/zabbix/zabbix.psk`.

    ![Configure host encryption](new-host-encrypt-configuration.png)

1.  Click **Add**  in the lower right corner of the configuration widget to complete the configuration task. Zabbix reloads the `Hosts` page, and the new `zabbixtarget` host is displayed. The `Availability` button displays the `Connection` status. A green button indicates an active connection. It takes a minute or two to establish the connection.

    {{< note >}}
    Due to the default polling interval, It might be necessary to refresh the page to update the information. If the button changes to `Red`, click on the button to review the root cause of the problem. Make any necessary changes based on the error message.
    {{< /note >}}
...
Configure the application using the web frontend. The Zabbix Dashboard highlights the state of the network. For a more comprehensive introduction, see the [Zabbix Quickstart](https://www.zabbix.com/documentation/6.0/en/manual/quickstart) tutorial.

### How to View Host Data Using Zabbix

The Zabbix frontend interface allows users to immediately view information about the new host. Follow these steps to see the information.

1.  To view data from the monitored host, click **Monitoring** on the side panel, then select **Latest Data**. This view provides an overview of all the data.
...
1.  Create on the **Create Item** button in the upper right corner of the UI.
...
1.  In this example, the new item monitors TCP port 80 on the remote host. Provide a name for the service, such as `HTTP Connectivity`.
    ...
    After adding all details, select **Add**.

    ![Configure a new item](configure-item.png)

1.  To see the new item, click on **Configuration**, **Hosts**, and **Links**. The new `HTTP Connectivity` item appears in the list. The item should be `Enabled`.

    ![View the new item](item-list-view.png)

1.  To view the new data, select **Monitoring** and **Latest Data** from the left menu. The new item appears in the list.

    ![View the value of the new item](monitoring-new-data.png)

1.  Click the **Graph** link to the right of the item to see the results. The graph shows a constant historical value of `1`. This means the port has continually been up and accepting connections.

    ![View a graph of the item](new-item-data.png)

### How to Configure Triggers

A Zabbix trigger alerts administrators to problems with a target. A trigger can generate a warning whenever a particular monitored value exceeds a threshold or matches a certain value. To create a trigger, follow these steps.

1.  Select **Configuration** and **Hosts**. Then select **Triggers** in the row corresponding to `zabbixtarget`.

    ![Select the trigger list](add-host-trigger.png)

1.  This displays a list of current triggers. Select **Create trigger** in the upper right corner.

    ![A list of current triggers](create-trigger.png)

1.  Zabbix displays a template to use to create a trigger. Provide the name `HTTP Down` for the trigger. Set the `Severity` to `High`.
...
1.  To ensure the trigger is working, generate the conditions required to activate it. In this case, shut down the web server to disable port `80`. Click **Monitoring** and **Problems** and wait for the alert to appear in the list of `Problems`. It can take up to a minute to raise the alert due to the `Update Interval`.

    {{< note >}}
    After connectivity returns, the status of the problem changes to `Resolved`. The `Problems` page contains filters to change the display criteria. `Recent Problems` displays all problems matching the timeline, including resolved issues. The `Problems` filter only lists the active issues.
    {{< /note >}}
...
1.  Enter the settings for the email relay server. The following settings illustrate the settings for the free [SMTP2Go](https://www.smtp2go.com/) SMTP relay service. Select **Update** after adding the information. Then click the **Test** link on the right side of the `Email` entry to send a test mail and confirm its reception.
    ...
    {{< note >}}
    Settings like `SMTP Server` and `SMTP server port` are server dependent. Consult the documentation for your SMTP server to determine the correct values. Most SMTP relay servers require a username and password. This information can be added to the `Authentication` section of the form.
    {{< /note >}}

    ![Configure the email server settings](configure-email-type.png)

1.  Create a new user to send the alerts to. First click on **Administration** and **Users**.

    ![Access the users menu](admin-users.png)

1.  To set up email for the `Admin` user, Click on the link labeled `Admin`.
...
1.  Select the **Media** tab, then click **Add**.

    ![Enter media details for the user](admin-add-media.png)

1.  Select `Email` for the type. Enter the email address under `Send To`. Restrict the `When Active` times if necessary. For `Use if Severity`, define the warnings that trigger an alert.
