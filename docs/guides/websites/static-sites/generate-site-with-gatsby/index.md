...
h1_title: "How to Generate Static Sites with Gatsby"
enable_h1: true
contributor:
  name: Nathaniel Stickman
...
With the Gatsby CLI installed, the process of setting up a new Gatsby project is straightforward. These next few sections cover doing just that, including applying a starter template. These give you a basis to start your Gatsby project, including a wide array of themes and layouts.

Later on, you can find explanations framing each of the main parts of a Gatsby project. Then keep reading to see how to build a customized website out of a template and, finally, how to deploy your ready website.
...
Check out [Gatsby Starter Library](https://www.gatsbyjs.com/starters/) to see a collection of starter templates to get started.
...
Now you can view the default website that comes with the starter template. Gatsby includes a development server, and while the server is not fit for production, it provides a convenient way to preview the static site.
...
- **Pages**: JSX files stored here automatically get converted into pages with their own paths, based on the file names. In the default starter, the `page-2.js` file in this subdirectory results in a `/page-2` path. The `index.js` file results in the homepage, with a path of `/`.

**Components**These renders to HTML elements, with each component functioning as a reusable portion of a page. A navigation bar is an example. Rather than maintaining the code for that piece on every distinct page, you can build the navigation menu as a component. Then it is easily reusable on any page on your website and can be maintained from a central location.
...
1. Take a look at the `src/pages/index.js` file. This file gets rendered into the blog's homepage, so modify it as you need. But keep in mind that much of the information about yourself and your website is sourced from the metadata adjusted above. For this example, none of the features actually needed to be altered.
...
1. Finally, create some blog posts. You can see the default posts stored in the `content``/blog`` subdirectory. Replace these subdirectories with subdirectories of your own, adding an `index.md` file within each for the Markdown content.
    ...
    Be sure to add frontmatter for each Markdown file. The starter supports the `title`, `date``, and `description` fields for the frontmatter by default. You can also adjust this by adjusting the GraphQL queries in the `src/pages/index.js` and `src/templates/blog-post.js` files and in the `graph-node.js` file.
