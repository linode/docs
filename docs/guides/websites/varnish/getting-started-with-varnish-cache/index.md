...
If your web server is nginx and you plan to use Varnish cache to serve WordPress, visit Linode's guide to [Using Varnish & nginx to Serve WordPress over SSL & HTTP on Debian 8](/cloud/guides/use-varnish-and-nginx-to-serve-wordpress-over-ssl-and-http-on-debian-8/).
...
These two are simply the minimum rule modifications. It is strongly recommended you use additional firewall rules on each, based on the other services you have running. If you're not sure how to set up a firewall, check out our guides on [iptables](/cloud/guides/control-network-traffic-with-iptables/) and [UFW](/cloud/guides/configure-firewall-with-ufw/).
