...
Before getting started, you'll need to make sure that your [hostname](/docs/guides/getting-started/#setting-the-hostname) has been properly set and that [static networking](/docs/guides/linux-static-ip-configuration/) has been configured **even if you only have one IP address**. You will also need to install MySQL. To do so, issue the following commands:
