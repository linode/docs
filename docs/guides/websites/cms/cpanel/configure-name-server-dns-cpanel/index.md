...
- **Branding**: Organizations can brand their name servers with their corporate or domain name. It provides the impression of being a larger and more professional organization.

Users should account for the time and complexity involved with managing their own DNS server. A self-hosted name server is easy to mis-configure and it is possible to omit an important step. Additionally, a self-hosted name server might be less secure than a third-party solution.
...
1.  Ensure the PowerDNS name server software is configured on WHM/cPanel. Using WHM, select **Nameserver Selection**. Ensure `PowerDNS` is selected.

    ![Ensure PowerDNS is enabled](WHM-PowerDNS.png)

1.  Ensure a custom domain for the subdomain and name servers has been created at one of the registrars.

2.  Log in to the registrar where the domain is registered. Create glue records for the custom name servers. These instructions differ between the registrars. Choose meaningful names for the name servers. By convention, name servers begin with `ns`, followed by a letter, the dot symbol `.`, and the domain name. For the domain `example.com`, the first name server could be `ns1.example.com`. Create at least two name servers for the domain. To get started, both name servers can point to the same system. Although this approach lacks redundancy, it is acceptable during the set-up stage.
...
The procedure for updating the official name servers for the domain varies between registrars. The high-level procedure involves selecting the domain record on the registrar and editing the name server settings. Consult the [Linode guide to registering custom DNS name servers](/docs/guides/custom-name-servers/#change-the-name-servers-for-your-domains) for a more detailed explanation. This guide contains links to the relevant instructions for the main domain name providers.
...
- **The glue records**: There should be an `A` record for each name server, pointing to the IP address.
- **The domain name servers**: The `AUTHORITY` section of the record should confirm the domain uses the self-hosted cPanel name servers.
- **The A record for the domain**: Retrieve the corresponding record from the name server and ensure it contains the IP address of the cPanel server.
...
The procedure for adding a secondary DNS service involves adding configuration to the cPanel server and provisioning the secondary service at another provider. The DNS records are transferred to the secondary using `AXFR` queries. cPanel must be configured to allow the transfers and requests.

### How to Configure cPanel/WHM to Support a Secondary DNS Server

Before proceeding with this section, ensure the custom domain is already configured in cPanel. Follow the steps in the "How to Set Up a Custom Domain for the cPanel Name Servers" to complete the configuration.
...
1.  Log in to the server hosting the cPanel/WHM installation. Edit the file named `/etc/pdns/pdns.conf`.
