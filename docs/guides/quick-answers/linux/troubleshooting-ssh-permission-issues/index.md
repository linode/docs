---
slug: troubleshooting-ssh-permission-issues
author:
  name: Linode Community
  email: docs@linode.com
description: 'This guide explains how to debug and resolve SSH publickey permission errors that prevent remote node access.'
og_description: 'This guide explains how to debug and resolve SSH publickey permission errors that prevent remote node access'
keywords: ['SSH','Troubleshooting','Permissions','Public Key']
license: '[CC BY-ND 4.0](https://creativecommons.org/licenses/by-nd/4.0)'
published: 2021-04-14
modified_by:
  name: Linode
title: "Troubleshooting SSH Permission Issues"
...
## Debugging and Fixing SSH Connection Errors

Before proceeding, you can extract some information about the failure and potentially work around the issue.

1.  Retrieve more information about the cause of the errors using the `vvv` option. Substitute your username and ip address of the target for the placeholder values. This command provides a log of the actions the client and target server take while trying to connect. A sample of the output is shown below.
...
3.  If you were able to gain access to your Linode in the previous step, follow the [Linode guide to SSH Public Key Authentication](/docs/security/authentication/use-public-key-authentication-with-ssh) to properly configure public key authentication.
...
...
    {{< /file >}}
5.  If you turned off public key authentication in the previous step, restart the SSH daemon to apply the change.

        systemctl restart sshd

### Resolving the Case Where the Client is Using the Incorrect Key.

If there are several keys on the client, it is possible SSH is choosing an old or incorrect key. It is possible to select a specific key using the `-i` option.

1.  Specify the exact key pair using the following command.

        ssh -i /path/to/key/id_rsa <accountname>@<ip_address>

### Resolving the Case Where the Client Does Not Possess the Correct Private Key
...
2.  Edit the file located at `/etc/ssh/sshd_config` and change the value of `PasswordAuthentication` to `Yes`. If you are not planning to generate and use a SSH private key in the near future, change `PubKeyAuthentication` to `No` at the same time.
...
### Resolving the Case Where the Target Server Does Not Have a Copy of the Public Key

This situation arises because the target server does not have your public key. Without that information, it cannot locate the key when it receives the SSH request. To correct this, copy the public key into the `authorized_keys` file in your directory on the Linode. For more information on how to generate a public key, see the [Linode guide](/docs/security/authentication/use-public-key-authentication-with-ssh).
...
2.  Inspect the contents of this file using either a text editor or the `cat` utility. A sample key is displayed below. Keep this file open for later.
        ...
        cat ~/.ssh/id_rsa.pub
    {{< caution >}}
Be careful to only share your public key. Your private key, which is usually named `id_rsa`, must always be kept secret.
    {{< /caution >}}
