...
Prior to beginning this guide for installing Redis, we assume that you have completed the steps outlined in our [getting started guide](/content/getting-started/). If you're new to Linux systems administration, we recommend that you read the guides in our [using Linux](/content/using-linux/) series, particularly the [administration basics guide](/content/using-linux/administration-basics).
...
Issue the following sequence of commands to download a basic init script, create a dedicated system user, mark this file as executable, and ensure that the Redis process will start following the next boot cycle:
    ...
    cd /opt/
    wget -O init-deb.sh http://www.linode.com/content/assets/629-redis-init-deb.sh
