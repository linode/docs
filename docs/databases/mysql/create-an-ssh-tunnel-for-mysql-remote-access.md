...
![Securely Administer MySQL with an SSH Tunnel](/content/assets/securely-administer-mysql-with-an-ssh-tunnel.png)
...
-   [MySQL](/content/hosting-website#install-mysql) is installed.
...
3.  Double-click the PuTTY file to begin - no need to install. You will see the following window:
    ...
    [![The session login screen in PuTTY on Windows.](/content/assets/361-putty-01-session.png)](/content/assets/361-putty-01-session.png)
...
7.  In the **Destination** field, enter `127.0.0.1:3306`. See the final configuration below:
    ...
    [![Tunneling a remote MySQL connection through SSH with PuTTY on Windows.](/content/assets/363-putty-04-mysql-ssh-tunnel.png)](/content/assets/363-putty-04-mysql-ssh-tunnel.png)
...
9.  If you haven't logged in to this system with PuTTY before, you will receive a warning similar to the following. Verify that this server is the one to which you want to connect, then click **Yes**:
    ...
    [![An unknown host key warning in PuTTY on Windows.](/content/assets/362-putty-02-host-key-warning.png)](/content/assets/362-putty-02-host-key-warning.png)
