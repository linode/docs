...
Before you begin installing and configuring the components described in this guide, please make sure you've followed our instructions for [setting your hostname](/content/getting-started#settingting-the-hostname). Issue the following commands to make sure it is set properly:
...
[![Phusion Passenger nginx installer program running on Ubuntu 10.10 (Maverick).](/content/assets/351-01-passenger-nginx-installer.png)](/content/assets/351-01-passenger-nginx-installer.png)
...
Nginx is now installed in `/opt/nginx`, but there are no "init" scripts to control this process. Issue the following sequence of commands to download a script, move it to the proper directory, set the proper permissions and set system startup links:
    ...
    cd /opt
    wget -O init-deb.sh http://www.linode.com/content/assets/602-init-deb.sh
...
If your application uses MySQL, install the database server by following our [MySQL on Ubuntu 10.10 (Maverick) guide](/content/databases/mysql/ubuntu-10-10-maverick). Once it's installed and configured properly, issue the following commands:
