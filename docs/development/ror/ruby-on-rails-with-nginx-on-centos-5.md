...
[![Phusion Passenger Nginx installer program running on CentOS 5.](/content/assets/352-01-passenger-nginx-installer.png)](/content/assets/352-01-passenger-nginx-installer.png)
...
Nginx is now installed in `/opt/nginx`, but there are no "init" scripts to control this process. Issue the following sequence of commands to download a script, move it to the proper directory, set the proper permissions and set system startup links:
    ...
    cd /opt
    wget -O init-rpm.sh http://www.linode.com/content/assets/603-init-rpm.sh
...
If your application uses MySQL, install the database server by following our [MySQL on CentOS 5 guide](/content/databases/mysql/centos-5). Once it's installed and configured properly, issue the following commands:
