...
[![Phusion Passenger Nginx installer program running on Ubuntu 9.04 (Jaunty).](/content/assets/464-01-passenger-nginx-installer.png)](/content/assets/464-01-passenger-nginx-installer.png)
...
Nginx is now installed in `/opt/nginx`, but there are no "init" scripts to control this process. Issue the following sequence of commands to download a script, move it to the proper directory, set the proper permissions and set system startup links:
    ...
    cd /opt
    wget -O init-deb.sh http://www.linode.com/content/assets/566-init-deb.sh
...
If your application uses MySQL, install the database server by following our [MySQL on Ubuntu 9.04 (Jaunty) guide](/content/databases/mysql/ubuntu-9-04-jaunty). Once it's installed and configured properly, issue the following commands:
