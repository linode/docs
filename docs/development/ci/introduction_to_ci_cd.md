...
external_resources:
 - '[Continuous Delivery](https://www.amazon.com/gp/product/B003YMNVC0)'
---

## What is Continuous Integration?

In software development, **integration** is the process of incorporating changes (new features, bug fixes, or refactoring) into the existing codebase. Traditionally, integration is performed when preparing for deployment of a new release. This approach can lead to problems, as integrating multiple or large changes in one go often results in last-minute conflicts and bugs. Several strategies have emerged to mitigate these problems. Continuous integration is one such solution: it creates a workflow where every commit made to the codebase is tested and built, and each developer on the project is expected to commit code at least once per day. Usually, both testing and building are automated, so that every commit to the central repository triggers an automatic build, which if successful then triggers a run of the test suite. In this way, conflicts or bugs are discovered quickly over the course of development, making the release process smoother and less painful.

## What is Continuous Delivery?

Continuous delivery takes these principles one step further: the automated build and test process is extended to every component of the application, including configuration files, database schemas, and environments. Whenever changes are made to any of these components, the build/test procedure is performed (ideally through an automated pipeline) so that the resulting build can be easily deployed. The resulting build does not always have to be released to production, but it should at least be deployed to an environment (such as staging) that is as close to production as possible. Product owners can therefore be confident that any build that makes it through this pipeline is releasable to the public. This approach offers several advantages:

  - New features and bug fixes can move from concept to deployment much more quickly.
  - Because the difference between each release is small, the risk of each individual deployment is reduced.
  - For the same reason, it is easier and safer to revert to a previous version in the event of problems.
...
These principles are taken from [Continuous Delivery](https://www.amazon.com/gp/product/B003YMNVC0) by Jez Humble and David Farley.
...
3.  **Keep everything in version control**: this includes makes it simple to revert to earlier versions, and provides a way to trigger the deployment pipeline whenever
4.  **Bring the pain forward**: the most painful and time-consuming aspects of development, which for most products include testing, integration, and deployment, should be done more frequently and earlier in the process.
...
![Simple Pipeline](/docs/assets/ci/simple-ci-pipeline.jpg)
...
![CD Pipeline](/docs/assets/ci/cd-pipeline.jpg)
...
Designing a custom pipeline can be time consuming, and as a result most development teams prefer to use one of the many professional continuous integration services available. These services often include GitHub/BitBucket integrations that allow you to abstract away the need to write manual Git hooks. Most of these services provide their tools through a cloud service; some, such as [Jenkins](https://jenkins.io), can also be self-hosted on a Linode. Our [How to Automate Builds with Jenkins on Ubuntu](https://linode.com/docs/development/ci/automate-builds-with-jenkins-on-ubuntu/) guide demonstrates how to set up a Jenkins server on a Linode, and introduces the syntax for constructing pipelines using a Jenkinsfile.
