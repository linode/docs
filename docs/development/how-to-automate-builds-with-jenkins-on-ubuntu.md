...
This guide is targeted toward DevOps professionals and thus presumes:
...
5.  The workstation and the remote Linode will each need Docker installed beforehand. Please read our [guide](/docs/applications/containers/how-to-install-docker-and-pull-images-for-container-deployment) for detailed instructions.
...
As you can see, the most basic process consists of three phases: build - test - deploy. Each time you make changes on your distributed version control system, you trigger an automation cycle on the Jenkins server. The entire set of instructions for running the process is on the `Jenkinsfile`, located at the root of your source repository. That single file tells the server *what* to do, *when* to do it and *how* you want those tasks to be performed.

## Write Your Example NodeJS Application

As mentioned in the previous section, the automation process starts by making a commit to a Version Control System. So, you must create a new repository in GitHub. 

This guide will use a simple NodeJS application to showcase how Jenkins' Pipelines operate. Select your `.gitignore` accordingly and don't forget to initialize it with a `README`:
...
This is a simplified test suite that uses `supertest` and `should`. It only has two tests: the first checks the HTTP status, expecting it to be 200. The second is not a real test but a control test that always passes.
...
4. Now, remove the container (you may need `sudo -i`), and run it again in detached mode to test the `JUnit` output. The `reports.xml` file should be saved afterward.
...
7.  Use the Linode Manager to reboot your server to apply these changes.
	...
	{{< caution >}}
It's beyond the scope of this guide to establish security parameters for Jenkins remote installation. However, be aware of these critical points that need to be addressed in a production environment:
...
1.  Use your browser to navigate to a default server address:
...
Both have support for continuous delivery and Jenkins plugins. Scripted syntax is based on the Groovy programming environment, so it is more complete and offers almost no limitations. On the other hand, Declarative syntax "was created to offer a simpler and more opinionated syntax for authoring Jenkins Pipeline" and thus is intended for everyday automation builds. You can learn more about syntax comparison in the Jenkins documentation [here.](https://jenkins.io/doc/book/pipeline/syntax/#compare)
...
Code blocks are delimited by curly brackets {}. No semicolons are used. Each statement has to be in its own line, and the heart of the `Jenkinsfile` are the steps you perform. Some common steps are:
...
* Almost any action you can think of is executable through Jenkins steps.
...
8. If you have multiple organization accounts along with your personal account, you will need to choose which organization contains your repository:
...
From here you can obtain valuable information regarding: 1) your build number, 2) the console output for each step, 3) selecting each stage for further analysis, 4) browsing through tabs with information about commit changes, tests results and artifacts stored, 5) replaying your build, 6) editing your pipeline visually, and 7) going to your pipeline settings.

### Automate your entire process with Jenkins

The `Jenkinsfile` template uses a very basic pipeline structure with only three stages. You can customize it to accommodate as many stages as needed. The final Pipeline structure is dictated by project complexity and the development guidelines you must follow. Since you've already walked through the NodeJS example, you know how to design a pipeline that automates each stage. For the purpose of this guide, the resulting pipeline should:
...
This complete Jenkinsfile is written using declarative syntax. If you read it carefully, you will notice that it describes the same procedure used during the application deployment completed in a previous section. This section will analyze the Jenkinsfile in more detail.

### Agent and environmental variables

The first block defines a globally available environmental variable called `DOCKER`. You can tell it's "global" because it is inside the pipeline block but outside the stages block. Next comes the `agent` - a statement that means Jenkins can use any (server) agent.
...
The first thing you notice about the `parallel` code block is that it is self-explanatory--it will run sub-stages in parallel. This feature is useful for building your two Docker images at the same with the very same shell commands you used before. Each image is declared in its own step, which is also part of an independent stage.
...
This stage introduces a different type of block: `when`. As the name implies, code is executed only if a certain condition is met. In this example, the code only runs if changes to the master branch are detected. Commits to other branches won't trigger this step of the pipeline.

Inside the steps you can optionally configure the `retry` and `timeout` arguments. The above example shows a nested usage where the image-build process has a timeout of 10 minutes and a total of three retries in case the timer expires.
...
4.  Return to your Pipeline view where you can watch how your stages run in parallel:
...
Up to this point, everything should work as expected without errors. But what happens when an error is encountered?
...
Blue Ocean's interface is still under development, which means that many aspects of Jenkins are not managed by the new interface. Below are some of the most common screens.
...
2.  If you click on `master` branch you will see a more detailed dashboard:
    ...
    From this view you can review lots of useful information like logs, artifacts, changes, test results, trends and much more.

## The Road Ahead

This guide has covered the basic automation workflow with Jenkins and Blue Ocean, but there is more you can do. Just to mention a few possibilities:
...
* The `post` (or any other section) can benefit from useful built-in functions like email, Slack, or HipChat notifications. As usual, you decide what will trigger the notifications, whether it's triggers from a successful build, a failure, a change, or a custom condition.
