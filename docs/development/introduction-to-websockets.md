...
*Long polling* (also known as Comet) is another popular communication method in which the client opens a connection with the server for a set duration. If the server does not have any information, it holds the request open until it has any information for the client, or until it reaches the end of a designated timeout. Essentially, Comet delays the completion of the HTTP response until the server has something to send to the client, a technique often called a *hanging-GET* or *pending-POST*.
...
{{< file-excerpt "websockets.js" javascript >}}
...
- **Open**: fires when the server responds to the WebSocket connection request, and indicates that the handshake has taken place and the connection is established. The callback to the open event is called `onopen`.
    {{< file-excerpt "websockets.js" javascript >}}
...
{{< /file-excerpt >}}
- **Message**: fires whenever the client receives data from the server. WebSocket messages contain the data from the server. The callback for the message event is `onmessage`.
{{< file-excerpt "websockets.js" javascript >}}
...
{{< /file-excerpt >}}
- **Error**: fires when there is any error in communication. The corresponding callback to the error event is `onerror`.
{{< file-excerpt "websockets.js" javascript >}}
...
{{< /note >}}
- **Close**: fires when the connection is closed. The corresponding callback to the close event is `onclose`.
{{< file-excerpt "websockets.js" javascript >}}
...
- **bufferedAmount**: bufferedAmount is also a read-only attribute. It represents the number of bytes of UTF-8 text that have been queued using the `send()` method. The following example demonstrates using this attribute to make sure that messages are sent only when the buffer is not full:
{{< file-excerpt "websockets.js" javascript >}}
...
{{< /file-excerpt >}}
- **protocol**: The protocol attribute lets the server know which protocol the client understands and can use over WebSocket.
{{< file-excerpt "websockets.js" javascript >}}
...
This section will combine the examples from the previous sections to create a simple, working WebSocket client application.

1. Using a text editor, create an `index.html` file and add the following content:
