...
4. Point your browser to http://192.0.0.0:9000, replacing `192.0.0.0` with the IP address or FQDN of your workstation Linode. You should see a screen similar to this:
...
6. If everything goes as expected you will be presented with the Portainer Dashboard.
...
5. Deploy your container when ready by hitting the corresponding button. The Nginx image will download automatically, it will be available for future deployments instantly. You will be directed to the **Containers** menu from where you can manage your containers.
...
1. Navigate to the **Containers** menu on the left Sidebar and click the check-box for the `web-01` container you just created. Notice you now have many options available to manage this container.

    ![Portainer Containers management](/docs/assets/webdev-remote-devices/linode-on-remote-devices-25.png)

2. You can | start | stop | kill | restart | pause | resume | remove | the selected container, in the same manner as you would use `docker` from the terminal. You can also edit `web-01` container by clicking on its name.
...
7. Swipe left on the **Connections** menu and create a new connection by touching the **+** sign.

    ![JuiceSSH New Connection](/docs/assets/webdev-remote-devices/linode-on-remote-devices-19.png)

8. Assign a Nickname to your connection, fill in the Linode's IP address or FQDN and save your connection touching the check mark on top right.
...
1. Editing simple configuration files is not hard. But, if you are serious about working remotely from your tablet then you will need a text editor capable of syntax highlighting among other things. Open QuickEdit and allow access to photos, media, and files on your device.
...
5. Enter your Linode's IP address or FQDN, desired label and username. Then select an authentication mode (you can use either password or SSH keys).

6. Browse to the location where you saved the `id_rsa` Private Key (during the JuiceSSH configuration) and select it. When you are ready save the connection.

7. Touch your Linode connection and you will be forwarded to the remote directory structure. Find your Nginx bind, mount and open the `test.json` file.
 ...
 * Interoperability between a desktop, a laptop or a tablet with total transparency. The current configuration allows you to manage your containers from a web interface and connect to your server through any SSH Client.
 * Centralized asset management: available from any device with an internet connection. The use of ownCloud allows you to access your assets from any device.
