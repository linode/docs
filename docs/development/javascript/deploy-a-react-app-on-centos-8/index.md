...
description: 'Learn to deploy a locally developed React application to your Linode using Rsync on CentOS 8.'
og_description: 'Use Rsync to deploy a React application from your local computer to a Linode on CentOS 8..'
...
modified_by:
  name: Linode
title: "Deploy a React Application on CentOS 8"
contributor:
  name: Phil Zona
...
3. Use SELinux’s `chcon` command to change the file security context for web content:

    sudo chcon -t httpd_sys_content_t /var/www/mydomain.com -R

    sudo chcon -t httpd_sys_rw_content_t /var/www/mydomain.com -R
...
1.  Restart the web server to apply the changes. Use whichever command applies to your web server:
